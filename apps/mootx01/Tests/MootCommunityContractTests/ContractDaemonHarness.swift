...
private final class ContractHarnessBundleProbe: NSObject {}
...
///   2. The current SwiftPM test product directory.
func findDaemonBinary() -> URL? {
    ...
    // 2. Walk upward from this test executable until reaching the SwiftPM
    // product directory. This binds the harness to the binary built in the
    // same scratch path and prevents a stale worktree's product from passing
    // (or hanging) the current candidate's gate.
    let roots = [
        URL(fileURLWithPath: CommandLine.arguments[0]),
        Bundle(for: ContractHarnessBundleProbe.self).bundleURL,
    ] +
        Bundle.allBundles.map(\.bundleURL) + Bundle.allFrameworks.map(\.bundleURL)
    for root in roots {
        var directory = root.resolvingSymlinksInPath().deletingLastPathComponent()
        for _ in 0..<12 {
            let candidate = directory.appendingPathComponent("mootx01-daemon-contract-host")
