...
struct CommunityTransferTests {
    ...
    func exportScopesReturnsRealCounts() async throws {
        ...
        let advertisedTokens = scopes.compactMap { scope -> String? in
            guard case .object(let fields) = scope,
                  case .string(let token) = fields["scopeToken"] else { return nil }
            return token
        }
        #expect(advertisedTokens == ["eligible-all"],
                "Community must advertise exactly its exportability-gated scope")
    }

    @Test("unadvertised export scopes are refused before a plan can become authority")
    func hiddenExportScopesAreRefused() async throws {
        let scratch = try await TransferScratch()
        defer { scratch.remove() }
        let coord = scratch.makeCoordinator()
        ...
        _ = try await scratch.kit.capture(
            scratch.handle,
            CaptureFrame(
                content: "Private row must never be reachable through a hidden scope",
                channel: .typed, room: "inbox",
                latticeAnchor: .udc("001"),
                addedBy: "test", embeddingModelID: "test",
        ...
        let bookmark = Data(scratch.destDirURL.absoluteString.utf8)
        for token in ["believed", "confirmed", "unconfirmed", "unknown"] {
            let fileName = "hidden-\(token).json"
            let result = await coord.exportPlan(
                bookmark: bookmark, fileName: fileName, scopeToken: token
            )
            #expect(structuredField(result, "outcome") == "failed")
            #expect(structuredField(result, "reason") == "policy-refused")
            #expect(!FileManager.default.fileExists(
                atPath: scratch.destDirURL.appendingPathComponent(fileName).path
            ))
        }
    }
