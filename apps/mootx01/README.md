...
1. Builds `mootx01-mcp` in release mode from `apps/mootx01/Package.swift`.
...
bash apps/mootx01/install.sh --local
```
...
```bash
bash apps/mootx01/uninstall.sh           # remove binary + config entries
bash apps/mootx01/uninstall.sh --purge   # also delete the MOOT data dir
bash apps/mootx01/uninstall.sh --local   # remove entry from ./.mcp.json (Claude Code only)
```
...
```bash
bash apps/mootx01/install.sh --no-permissions
```
...
on next use) and re-run `bash apps/mootx01/install.sh` to add the ARIA
permissions.
...
bash apps/mootx01/Tests/test_install_sh.sh
```
...
- It does not modify `apps/aria-mcp-server/Sources/aria-mcp/AriaMCPMain.swift`.
