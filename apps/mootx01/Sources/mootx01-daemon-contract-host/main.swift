...
func runContractHost() async -> Int32 {
    ...
    let providerVector = MootDaemonProvider.ProviderVersionVector.current
    descriptor.descriptorMAC = MootDaemonProvider.ProviderVersionVector.schema3MAC(
        descriptor: descriptor,
        vector: providerVector,
        installationRoot: testRoot
    )

    let descriptorData = DescriptorPublisher.encode(descriptor, vector: providerVector)
