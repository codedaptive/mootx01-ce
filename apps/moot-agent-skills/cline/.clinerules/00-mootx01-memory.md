...
4. Use `moot_fact_search` for structured facts.
5. Use `moot_list_lenses`, `moot_lens_*`, and `moot_synthesize` before spending many tokens on manual analysis.
6. Write durable knowledge with `moot_file_memory`, `moot_file_fact`, `moot_link_memories`, and `moot_write_journal`.
7. Correct trust state with confirm/update/withdraw/retire tools.
