#!/usr/bin/env bash
#
# submit-results.sh — run MatrixSprint and bundle the output into a
...
#
# Usage:
#   apps/MatrixSprint/submit-results.sh                  # full sweep
#   apps/MatrixSprint/submit-results.sh --quick          # smoke run
#   apps/MatrixSprint/submit-results.sh --tag MY-RIG     # override hw tag
#
...
echo "==> MatrixSprint bundle: $BUNDLE"
echo "==> $(date)"
echo ""
...
echo "==> system report"
{
    echo "MatrixSprint submission — system report"
...
# MatrixSprint submission — ${TAG} — ${DATE}

Hardware tag: \`${TAG}\`
Bundle path:  \`apps/MatrixSprint/${BUNDLE}/\`
...
\`\`\`sh
apps/MatrixSprint/submit-results.sh --tag ${TAG}$( [ -n "$QUICK" ] && echo " --quick" )
\`\`\`
...
echo "  3. git add apps/MatrixSprint/$BUNDLE"
