...
// MACD-3B5: HTTPTransport and its private helper types (BoundedLoopbackResponse,
// BoundedResponseReadError, LoopbackSocketOwner) are macOS-only — they implement
// loopback-HTTP transport to a resident daemon. iOS is embedded-only (MACD-3a
// EstatePlatformCapability.embeddedOnly); it has no resident daemon to reach.
// LoopbackSocketOwner is defined in FirstPartyDaemonAuthenticator.swift (also guarded),
// so this guard must be consistent with that one.
#if os(macOS)
