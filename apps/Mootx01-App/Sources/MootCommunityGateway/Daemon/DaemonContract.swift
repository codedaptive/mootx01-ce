// MACD-3B5: MootDaemonProvider is a macOS-only artifact (SecCode/ServiceManagement APIs absent
// from the iOS SDK). iOS is embedded-only per MACD-3a EstatePlatformCapability.embeddedOnly;
// it has no resident provider and must not compile daemon-contract code at all. The entire
// file is guarded so the iOS scheme sees an empty compilation unit.
#if os(macOS)
import AriaMCPWire
import Foundation
...
public enum DaemonContract {
    ...
    ///
    /// Schema 3 (MACD-3B4) adds the `ProviderVersionVector` companion type.
    /// The vector travels ALONGSIDE the descriptor (not embedded in it — Kong
    /// ruling: DaemonDescriptor stays at 16 fields). A schema-3 descriptor's
    /// MAC covers both `descriptor.macInput()` (length-prefixed) and the 7
    /// Wave-1 vector scalar fields; `verifySchema3MAC` must be used instead of
    /// `canonical.verifyMAC` for any schema-3 descriptor.
    public static let schemaVersion = 3

    /// The management revision this client implements. Compared against the
    /// provider's `managementRevisionMinimum`/`managementRevisionMaximum` range
    /// during the 4-axis compatibility check introduced in MACD-3B4.
    ///
    /// Wave 1 value = 1 (first stable management revision). Bumped when the
    /// management protocol between the app and the provider changes in a way
    /// that is not backward-compatible with older provider builds.
    public static let managementRevision: UInt64 = 1
...
public enum DaemonDescriptorDefect: String, Sendable, Equatable, CaseIterable {
    ...
    /// A schema-3 descriptor arrived without a companion `ProviderVersionVector`.
    /// Schema-3 compatibility evaluation requires the vector; its absence means
    /// the descriptor is incomplete and cannot be acted on.
    case missingVersionVector
}
...
public enum DaemonCompatibility: Sendable, Equatable {
    ...
    /// The descriptor is structurally sound but the companion `ProviderVersionVector`
    /// failed the 4-axis compatibility check. The associated verdict names which axis
    /// closed: `keepOwnerNoOverlap` (management mismatch), `updateApp` (data-plane
    /// mismatch), `generationDowngrade` (release generation is zero), or
    /// `candidateCannotReadEstate` (estate schema out of range — not checked at connect
    /// time, reserved for the takeover coordinator in Part B).
    case vectorIncompatible(VersionCompatibilityVerdict)
}
...
public struct DaemonCompatibilityPolicy: Sendable, Equatable {
    ...
    /// The management revision this client implements (from `DaemonContract.managementRevision`).
    /// Used in the 4-axis vector check: the provider's `managementRevisionMinimum` must be ≤
    /// this value ≤ `managementRevisionMaximum`, or the descriptor is refused with
    /// `.vectorIncompatible(.keepOwnerNoOverlap)`.
    public let managementRevision: UInt64
    ...
    public init(
        ...
        managementRevision: UInt64 = DaemonContract.managementRevision
    ) {
        self.providerIdentifier = providerIdentifier
        self.serviceIdentifier = serviceIdentifier
        self.schemaVersion = schemaVersion
        ...
        self.managementRevision = managementRevision
    }
    ...
    public static let current = DaemonCompatibilityPolicy(
        ...
        managementRevision: DaemonContract.managementRevision
    )
    ...
    /// Judge one descriptor and its schema-3 companion vector.
    ///
    /// Calls the base `evaluate(_:)` first; if that returns `.compatible`, applies
    /// the 4-axis version-vector check:
    ///
    /// 1. **Vector presence**: nil vector on a schema-3 descriptor → `.invalidDescriptor(.missingVersionVector)`.
    /// 2. **Management overlap**: `v.managementRevisionMinimum ≤ managementRevision ≤ v.managementRevisionMaximum` —
    ///    failure → `.vectorIncompatible(.keepOwnerNoOverlap)`.
    /// 3. **Data-plane overlap**: `v.dataPlaneRevisionMinimum ≤ UInt64(contractRevision) ≤ v.dataPlaneRevisionMaximum` —
    ///    failure → `.vectorIncompatible(.updateApp)`.
    /// 4. **Release generation**: `v.providerReleaseGeneration > 0` —
    ///    failure → `.vectorIncompatible(.generationDowngrade)`.
    ///
    /// Estate schema is NOT checked at connect time — that check belongs to the
    /// takeover coordinator (Part B) which knows the live estate schema.
    ///
    /// **Security note (B4/Perkins A2):** `v.migrationTargetSchema` and
    /// `v.capabilityRevisions` are NOT evaluated here and MUST NOT be used as
    /// decision inputs. They are Wave-2 Swift-type-only fields whose MAC coverage
    /// is not yet complete. Consuming them before the MAC covers them would allow
    /// an attacker to carry in a forged value through the unauthenticated portion
    /// of the wire record.
    ///
    /// - Parameters:
    ///   - descriptor: The record to judge.
    ///   - vector: The companion version vector, or nil when absent.
    /// - Returns: `.compatible`, the first structural gate that closed, or a
    ///   vector axis verdict.
    public func evaluate(_ descriptor: DaemonDescriptor, vector: ProviderVersionVector?) -> DaemonCompatibility {
        let base = evaluate(descriptor)
        guard base == .compatible else { return base }
        guard let v = vector else {
            return .invalidDescriptor(.missingVersionVector)
        }
        // Management revision overlap: this client's managementRevision must fall
        // within the provider's declared management revision range.
        guard v.managementRevisionMinimum <= managementRevision,
              managementRevision <= v.managementRevisionMaximum else {
            return .vectorIncompatible(.keepOwnerNoOverlap)
        }
        // Data-plane revision overlap: the contract revision (UInt64 for comparison)
        // must fall within the provider's data-plane revision range.
        let contractRev = UInt64(contractRevision)
        guard v.dataPlaneRevisionMinimum <= contractRev,
              contractRev <= v.dataPlaneRevisionMaximum else {
            return .vectorIncompatible(.updateApp)
        }
        // Release generation must be non-zero. A generation of 0 is a sentinel value
        // that indicates a descriptor published before the version vector existed or
        // a downgrade to a pre-MACD-3B1 provider.
        guard v.providerReleaseGeneration > 0 else {
            return .vectorIncompatible(.generationDowngrade)
        }
        return .compatible
    }

    /// Judge a descriptor against both a generation pair and a schema-3 version vector.
    ///
    /// Convenience overload combining the generation monotonicity check with the
    /// 4-axis vector check. Calls `evaluate(_:lastCredentialGeneration:lastDescriptorGeneration:)`
    /// first; if that returns `.compatible`, applies the vector check via `evaluate(_:vector:)`.
    ///
    /// - Parameters:
    ///   - descriptor: The record to judge.
    ///   - vector: The companion version vector, or nil when absent.
    ...
    public func evaluate(
        ...
        let generationResult = evaluate(
            descriptor,
            lastCredentialGeneration: lastCredentialGeneration,
            lastDescriptorGeneration: lastDescriptorGeneration
        )
        guard generationResult == .compatible else { return generationResult }
        return evaluate(descriptor, vector: vector)
    }
