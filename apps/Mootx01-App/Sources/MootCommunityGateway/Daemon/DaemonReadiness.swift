...
// it has no resident provider and must not compile daemon-readiness code at all. The entire
...
public actor DaemonReadiness {
    ...
    /// Reads the daemon's published descriptor and its companion version vector.
    ///
    /// Returns nil when no descriptor exists; throws when one exists but cannot
    /// be decoded. Both cases map to `.unavailable`. Returns a pair so the vector
    /// travels with the descriptor without adding stored fields to `DaemonDescriptor`
    /// (Kong ruling, MACD-3B4 BRR: DaemonDescriptor stays at 16 fields).
    ///
    /// The vector is `nil` on a schema-2 descriptor (no vector present); it is
    /// always non-nil for a schema-3 descriptor published by a MACD-3B1+ provider.
    public typealias DescriptorLoader = () async throws -> (DaemonDescriptor, ProviderVersionVector?)?

    /// Establishes an authenticated session. Throwing is the only way to say
    /// "no": no return value means unauthenticated. The vector companion is
    /// passed alongside the descriptor so the authenticator can verify the
    /// schema-3 MAC without reaching back into global state.
    public typealias Authenticator = (DaemonDescriptor, ProviderVersionVector?) async throws -> AuthenticatedDaemonTransport
    ...
    public func connect() async -> DaemonReadinessState {
        ...
        // omission.
        //
        // The vector-aware, generation-aware overload is used: it supplies both
        // the monotonicity history and the schema-3 4-axis vector check.
        ...
        case .invalidDescriptor, .missingCapabilities, .vectorIncompatible:
            // Vector incompatibility maps to .incompatible — the caller should
            // check DaemonCompatibilityPolicy directly for a detailed verdict
        ...
        // credential attempt. The vector is passed through so the authenticator
        // can verify the schema-3 MAC without re-fetching the descriptor.
        let authenticated: AuthenticatedDaemonTransport
        do {
            authenticated = try await authenticate(descriptor, vector)
        } catch {
