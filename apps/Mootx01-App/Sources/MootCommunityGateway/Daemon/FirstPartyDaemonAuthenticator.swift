...
// it has no resident provider and therefore needs no daemon authenticator. The entire file is
// guarded so the iOS scheme sees an empty compilation unit.
#if os(macOS)
import AriaMCPWire
import Foundation
...
public struct FirstPartyDaemonAuthenticator: Sendable {
    ...
    ///
    /// Schema-3 MAC path (MACD-3B4): requires both a schema-3 descriptor AND a
    /// non-nil `ProviderVersionVector`. A missing vector fails closed. The MAC
    /// is verified via `ProviderVersionVector.verifySchema3MAC`, which covers
    /// both the schema-2 macInput() contribution (length-prefixed) and the 7
    /// Wave-1 vector scalar fields.
    ///
    /// **Security (B4/Perkins A2 fail-closed refusal):** `v.migrationTargetSchema`
    /// and `v.capabilityRevisions` are NOT evaluated and MUST NOT be read here.
    /// Those fields have no MAC coverage in Wave 1 and are attacker-controllable
    /// on the wire. They become decision inputs only in Wave 2, after their MAC
    /// extension tail is complete.
    ///
    /// - Parameters:
    ///   - descriptor: The published descriptor. Untrusted on entry.
    ///   - vector: The companion version vector for schema-3 descriptors.
    /// - Returns: An authenticated transport.
    public func authenticate(
        _ descriptor: DaemonDescriptor,
        vector: ProviderVersionVector?
    ) async throws -> AuthenticatedDaemonTransport {
        // GATE 1 — schema check, then authenticity, then compatibility.
        // All three before any traffic. A schema mismatch is refused first so
        // the wrong MAC path is never attempted (schema-2 vs schema-3 MAC
        // input layouts differ; calling verifyMAC on a schema-3 descriptor
        // always returns false, which is correct but misleading).
        guard descriptor.schemaVersion == DaemonContract.schemaVersion else {
            throw FirstPartyAuthenticationError.descriptorNotAuthentic
        }
        guard let v = vector else {
            // A schema-3 descriptor without a companion vector cannot be
            // verified — the schema-3 MAC covers the vector fields.
            throw FirstPartyAuthenticationError.descriptorNotAuthentic
        }
        let root: [UInt8]
        do {
        ...
        // Schema-3 MAC: covers length-prefixed macInput() + 7 vector scalar fields.
        // Do NOT call canonical.verifyMAC() here — that verifies only the schema-2
        // macInput() bytes and always returns false for a schema-3 descriptor.
        ...
        let verdict = policy.evaluate(descriptor, vector: vector)
