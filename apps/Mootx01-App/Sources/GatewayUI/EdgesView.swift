...
// the same drawers; loopback-HTTP transport is live, Bonjour/LAN discovery is the remaining A2 step).
//
...
struct EdgesView: View {
    ...
    private func stateBadge(_ state: AdapterState) -> some View {
        // state.rawValue is substrate-side vocabulary from MootGateway;
        // surfaced verbatim as a status chip.
        let color: Color = {
            switch state {
            case .live: return .green
            case .seam: return .orange
            case .pendingRegistration: return .blue
            // v1.1-deferred surfaces: deliberately not built in beta by ruling.
            case .deferredToV1_1: return .purple
            }
        }()
    ...
    private var crossProcess: some View {
        GroupBox(String(localized: "Cross-process demo (shared estate, external client)")) {
            VStack(alignment: .leading, spacing: 8) {
                Text(String(localized: "This estate is a SQLite file. Point a real aria-mcp at the same file and an external MCP client (Claude Code) sees the same drawers — proving the substrate is shared. The loopback-HTTP transport (GatewayTransport.swift) is implemented; LAN discovery via Bonjour is the remaining A2 step."))
    ...
    private func commandText(_ path: String) -> String {
        "ARIA_MCP_SQLITE_PATH=\(path) \\\n  swift run --package-path apps/aria-mcp-server aria-mcp"
    }
