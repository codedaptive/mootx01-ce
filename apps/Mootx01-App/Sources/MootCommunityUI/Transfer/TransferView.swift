...
// refused-plan states carry an accessibility value with the humanized daemon
// reason; disabled controls include an accessibility hint explaining the
// prerequisite. String(localized:) for all display strings — zero
// unlocalized text.
//
// Human-language law (R-C20): every daemon reason code is presented through
// CommunityDaemonReason — the human explanation is the visible caption and
// the accessibility value; the machine code renders only as the labeled
// selectable "Reason code:" caption.
//
...
public struct TransferView: View {
    ...
    private var importJobStateView: some View {
        ...
        case .failed(let reasonCode, let partial):
    ...
    private var importControlsGroup: some View {
        ...
        if case .failed(let reasonCode) = model.lastImportPlanOutcome {
            let presented = CommunityDaemonReason(code: reasonCode)
            VStack(alignment: .leading, spacing: 2) {
                Text(String(localized: "transfer.outcome.import.plan.failed \(presented.explanation)"))
                    .font(.caption)
                    .foregroundStyle(.red)
                    .accessibilityValue(presented.explanation)
                reasonCodeDetail(presented)
        ...
        // Import execute outcome surfacing.
        // .submitted leads to a job ID and status display above; the user
        // sees it. .denied and .failed get inline feedback so the button
        // never looks like it did nothing. The permission-loss-at-execute
        // case (.denied) is especially critical to surface because the user
        // prepared a plan and expects a job to start.
        if let outcome = model.lastImportExecuteOutcome {
            ...
            case .denied(let reasonCode):
                ...
                VStack(alignment: .leading, spacing: 2) {
                    Label(
                        String(localized: "transfer.outcome.import.execute.denied \(presented.explanation)"),
                        systemImage: "hand.raised"
                    )
                    .font(.caption)
                    .foregroundStyle(.red)
                    .accessibilityLabel(
                        String(localized: "transfer.outcome.import.execute.denied.a11y \(presented.explanation)")
                    )
                    .accessibilityValue(presented.explanation)
                    reasonCodeDetail(presented)
                }
            case .failed(let reasonCode):
                let presented = CommunityDaemonReason(code: reasonCode)
                VStack(alignment: .leading, spacing: 2) {
                    Text(String(localized: "transfer.outcome.import.execute.failed \(presented.explanation)"))
                        .font(.caption)
                        .foregroundStyle(.red)
                        .accessibilityValue(presented.explanation)
                    reasonCodeDetail(presented)
        ...
        if let outcome = model.lastCancelOutcome {
            ...
            case .failed(let reasonCode):
                let presented = CommunityDaemonReason(code: reasonCode)
                VStack(alignment: .leading, spacing: 2) {
                    Text(String(localized: "transfer.outcome.cancel.failed \(presented.explanation)"))
                        .font(.caption)
                        .foregroundStyle(.red)
                        .accessibilityValue(presented.explanation)
                    reasonCodeDetail(presented)
    ...
    private var exportControlsGroup: some View {
        ...
        if case .failed(let reasonCode) = model.lastExportPlanOutcome {
            let presented = CommunityDaemonReason(code: reasonCode)
            VStack(alignment: .leading, spacing: 2) {
                Text(String(localized: "transfer.outcome.export.plan.failed \(presented.explanation)"))
                    .font(.caption)
                    .foregroundStyle(.red)
                    .accessibilityValue(presented.explanation)
                reasonCodeDetail(presented)
        ...
        if let outcome = model.lastExportExecuteOutcome {
            ...
            case .denied(let reasonCode):
                ...
                VStack(alignment: .leading, spacing: 2) {
                    Label(
                        String(localized: "transfer.outcome.export.execute.denied \(presented.explanation)"),
                        systemImage: "hand.raised"
                    )
                    .font(.caption)
                    .foregroundStyle(.red)
                    .accessibilityLabel(
                        String(localized: "transfer.outcome.export.execute.denied.a11y \(presented.explanation)")
                    )
                    .accessibilityValue(presented.explanation)
                    reasonCodeDetail(presented)
                }
            case .failed(let reasonCode):
                let presented = CommunityDaemonReason(code: reasonCode)
                VStack(alignment: .leading, spacing: 2) {
                    Text(String(localized: "transfer.outcome.export.execute.failed \(presented.explanation)"))
                        .font(.caption)
                        .foregroundStyle(.red)
                        .accessibilityValue(presented.explanation)
                    reasonCodeDetail(presented)
    ...
    private func jobStatusRefreshOutcomeView(_ outcome: TransferJobStatusOutcome?) -> some View {
        ...
        case .failed(let reasonCode):
            let presented = CommunityDaemonReason(code: reasonCode)
            VStack(alignment: .leading, spacing: 2) {
                Text(String(localized: "transfer.outcome.job.status.failed \(presented.explanation)"))
                    .font(.caption)
                    .foregroundStyle(.red)
                    .accessibilityValue(presented.explanation)
                reasonCodeDetail(presented)
            ...
            if let receipt {
                HStack {
                    Text(String(localized: "transfer.counts.receipt.label"))
                        .foregroundStyle(.secondary)
                        .font(.caption)
                    // Receipt is daemon-issued opaque string; display as
                    // monospaced and selectable so it can be copied into a
                    // support request (R-C21).
                    Text(receipt)
                        .textSelection(.enabled)
