...
public final class CommunityAppModel {
    ...
    /// The Recall pane's decoded state (census com.recall.result R-C6,
    /// com.recall.empty R-C7). Starts idle; every completed call lands as a
    /// `CommunityRecallOutcome`, never as a bare wire-reply string.
    private(set) var recallOutcome: CommunityRecallOutcome = .idle
    ...
    public var status: String {
        ...
        case .blocked(let reason):
            // The machine reason code passes through the closed human
            // mapping; the slug itself never reaches the status line (R-C1).
            return CommunityDaemonReason(code: reason).explanation
        case .ready:
    ...
    /// Labeled technical detail for the blocked connection state: the exact
    /// machine reason code, rendered by the shell as a secondary, selectable
    /// line so support requests keep the daemon's precise word (R-C1).
    /// nil for every other state — they carry no wire reason.
    public var statusTechnicalDetail: String? {
        guard case .blocked(let reason) = connectionState else { return nil }
        return CommunityDaemonReason(code: reason).technicalDetail
    }
    ...
    public func recall() async {
        ...
        // The reply's structured rows become the pane's human rendering; the
        // verbatim wire text survives inside the outcome for the labeled
        // "Estate reply" disclosure (census com.recall.result R-C6,
        // com.recall.empty R-C7).
        recallOutcome = CommunityRecallPresentation.outcome(of: result)
    }
