                ...
                Section(String(localized: "Privacy")) {
                    Picker(String(localized: "Sensitivity"), selection: $model.sensitivity) {
                        ForEach(choices.sensitivities) { sensitivity in
                            // The humanized display name; the wire value never
                            // reaches the row label (census R-C2).
    ...
    private var outcomeView: some View {
        ...
        case .applied(let receipt):
            Section(String(localized: "Captured")) {
                ...
                // Humanized sensitivity, matching the picker rows (census R-C3).
                Text(receipt.effectivePolicy.sensitivity.displayName)
        ...
        case .refused(let field, let reasonCode):
            // The refused setting and the daemon's reason, both in human
            // language (census R-C4/R-C5). The exact machine code stays
            // available as a labeled, selectable technical detail.
