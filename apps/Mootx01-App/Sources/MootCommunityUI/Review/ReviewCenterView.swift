...
private struct ReviewModeRowView: View {
        ...
        NavigationLink(destination: sessionDestination) {
            HStack {
                VStack(alignment: .leading, spacing: 4) {
                    ...
                    // Blocked modes keep the daemon's exact reason code as a
                    // labeled, selectable detail under the human sentence
                    // (R-C14); the code is never the primary status language.
                    if case .blocked(let reason) = status {
                        Text(CommunityDaemonReason(code: reason).technicalDetail)
    ...
    private var statusLabel: String {
        ...
        // The daemon's reason code becomes a human sentence (R-C14).
        case .blocked(let reason):   CommunityDaemonReason(code: reason).explanation
        }
    }
    ...
    var body: some View {
        Group {
            if model.isLoadingSession {
                ...
                // exact code stays as a labeled, selectable detail (R-C15).
                let presented = CommunityDaemonReason(code: reason)
                ContentUnavailableView {
                    Label(String(localized: "Session Unavailable"),
                ...
                .accessibilityLabel(
                    String(localized: "Session blocked: \(presented.explanation)"))
            } else {
                ContentUnavailableView(
...
private struct DuplicateGroupRowView: View {
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 8) {
            ...
            // Involved records — a human count as the primary description
            // (CONTRACT-04 carries no record subjects; census CD-3), then the
            // full identifiers, labeled and copyable, so the user can inspect
            // the exact records the daemon grouped.
            Text(ReviewDuplicatePresentation.involvedSummary(group.involvedRecordIDs))
                .font(.caption)
                .foregroundStyle(.secondary)
            Text(ReviewDuplicatePresentation.recordIDLine(group.involvedRecordIDs))
                .font(.caption2.monospaced())
                .foregroundStyle(.secondary)
                .textSelection(.enabled)
                .accessibilityLabel(
                    ReviewDuplicatePresentation.recordIDsAccessibilityLabel(
                        count: group.involvedRecordIDs.count))
...
// MARK: - ReviewDuplicatePresentation

/// Display strings for one duplicate group's involved records (census
/// com.review.session.dup.records).
///
/// CONTRACT-04 carries only record UUIDs for a duplicate group — no subjects
/// or titles (census contract deficiency CD-3). The truthful presentation is
/// therefore a human count as the primary description, with the full record
/// identifiers demoted to an explicitly labeled, selectable secondary line.
/// Identifiers are never truncated: a truncated identifier can be neither
/// verified nor copied, so it would disguise information rather than carry it.
enum ReviewDuplicatePresentation {
    /// Primary human description: how many records the daemon grouped.
    static func involvedSummary(_ ids: [UUID]) -> String {
        String(localized: "review.duplicates.count \(ids.count)")
    }

    /// Labeled, selectable list of the full record identifiers.
    static func recordIDLine(_ ids: [UUID]) -> String {
        let list = ids.map { $0.uuidString }.joined(separator: ", ")
        return String(localized: "review.duplicates.record.ids \(list)")
    }

    /// VoiceOver label for the identifier line — states what the line holds
    /// without reading every character of every UUID aloud.
    static func recordIDsAccessibilityLabel(count: Int) -> String {
        String(localized: "review.duplicates.record.ids.a11y \(count)")
    }
}
        ...
        HStack(spacing: 12) {
            ...
            VStack(alignment: .leading, spacing: 2) {
                Text(String(localized: "review.completion.failed.title"))
                    ...
                    .accessibilityAddTraits(.isHeader)
                // Human body first; the daemon's reason (a machine slug under
                // CD-1) follows as labeled, selectable technical detail.
                Text(ReviewOutcomePresentation.completionFailureBody)
                    .font(.caption)
                    .foregroundStyle(.secondary)
                Text(ReviewOutcomePresentation.completionFailureDetail(reason: reason))
                    .font(.caption2)
                    .foregroundStyle(.secondary)
                    .textSelection(.enabled)
            }
...
private struct ActionOutcomeBanner: View {
    ...
    var body: some View {
        HStack(spacing: 12) {
            Image(systemName: outcomeIcon)
                ...
                .accessibilityHidden(true)
            VStack(alignment: .leading, spacing: 2) {
                Text(ReviewOutcomePresentation.message(for: outcome))
                    .font(.subheadline)
                    .foregroundStyle(.primary)
                // The daemon's own reason (a machine slug under CD-1) as
                // labeled, selectable technical detail — never interpolated
                // into the human sentence.
                if let detail = ReviewOutcomePresentation.detail(for: outcome) {
                    Text(detail)
                        .font(.caption)
                        .foregroundStyle(.secondary)
                        .textSelection(.enabled)
    ...
    // a success indicator for a non-success outcome. The banner's display
    // strings live in ReviewOutcomePresentation below so they are assertable
...
// MARK: - ReviewOutcomePresentation

/// Human-language display strings for daemon action outcomes and completion
/// failures (census com.review.outcome.banner, com.review.completion.failure).
///
/// CONTRACT-04's conflict/refused/failed cases carry the daemon's own reason
/// string, which in practice is a machine slug (census contract deficiency
/// CD-1). The primary banner line is therefore a human sentence derived from
/// the outcome CASE — which the contract does guarantee — and the daemon's
/// reason is preserved verbatim on a labeled, selectable detail line rather
/// than interpolated into the sentence.
enum ReviewOutcomePresentation {
    /// The primary human sentence for a daemon action outcome.
    static func message(for outcome: ReviewActionOutcome) -> String {
        switch outcome {
        case .applied:
        ...
        case .conflict:
            String(localized: "review.outcome.conflict")
        case .staleSession:
            String(localized: "Session is stale — please reconnect")
        case .refused:
            String(localized: "review.outcome.refused")
        case .failed:
            String(localized: "review.outcome.failed")
        }
    }

    /// The daemon's own reason, labeled as technical detail. `nil` for the
    /// outcomes that carry no reason string.
    static func detail(for outcome: ReviewActionOutcome) -> String? {
        switch outcome {
        case .applied, .alreadyApplied, .staleSession:
            nil
        case .conflict(let reason), .refused(let reason), .failed(let reason):
            String(localized: "review.outcome.detail \(reason)")
        }
    }

    /// The completion-failure banner's human body. Its title stays
    /// "review.completion.failed.title", owned by CompletionFailureBanner.
    static var completionFailureBody: String {
        String(localized: "review.completion.failed.message")
    }

    /// The daemon's completion-failure reason, labeled as technical detail.
    static func completionFailureDetail(reason: String) -> String {
        String(localized: "review.outcome.detail \(reason)")
    }
}
