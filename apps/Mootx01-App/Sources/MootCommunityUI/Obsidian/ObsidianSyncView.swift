...
// blocked and failed states carry an accessibility value with the humanized
// daemon reason. String(localized:) for all display strings — zero
// unlocalized text.
//
// Human-language law (R-C19): every daemon reason code is presented through
...
public struct ObsidianSyncView: View {
        ...
        GroupBox(label: Text(String(localized: "obsidian.section.authorization"))) {
            VStack(alignment: .leading, spacing: 8) {
                if let authState = model.authorizationState {
                    ...
                    case .needsRenewal(_, let name, let reasonCode):
                ...
                // Surface vault selection denial with the humanized reason.
                ...
                if case .denied(let reasonCode) = model.lastVaultSelectionOutcome {
                    let presented = CommunityDaemonReason(code: reasonCode)
                    VStack(alignment: .leading, spacing: 2) {
                        Label(
                            String(localized: "obsidian.outcome.vault.denied \(presented.explanation)"),
                            systemImage: "hand.raised"
                        )
                        .font(.caption)
                        .foregroundStyle(.red)
                        .accessibilityLabel(
                            String(localized: "obsidian.outcome.vault.denied.a11y \(presented.explanation)")
                        )
    ...
    private func syncStatusContent(_ status: ObsidianSyncStatus) -> some View {
        ...
        case .interrupted(let reasonCode, _):
    ...
    private var controlsSection: some View {
        ...
        // Enable outcome non-success surfacing: a daemon-refused Enable
        // (e.g. authorization missing, policy violation) gets inline feedback
        // so the user can distinguish it from a no-op. .enabled causes a
        // status reload that makes success visible; only .refused and
        // .failed need inline feedback here.
        if let outcome = model.lastEnableOutcome {
            ...
            case .refused(let reasonCode):
                ...
                VStack(alignment: .leading, spacing: 2) {
                    Label(
                        String(localized: "obsidian.outcome.enable.refused \(presented.explanation)"),
                        systemImage: "hand.raised"
                    )
                    .font(.caption)
                    .foregroundStyle(.red)
                    .accessibilityLabel(
                        String(localized: "obsidian.outcome.enable.refused.a11y \(presented.explanation)")
                    )
                    .accessibilityValue(presented.explanation)
                    reasonCodeDetail(presented)
                }
            case .failed(let reasonCode):
                let presented = CommunityDaemonReason(code: reasonCode)
                VStack(alignment: .leading, spacing: 2) {
                    Text(String(localized: "obsidian.outcome.enable.failed \(presented.explanation)"))
                        .font(.caption)
                        .foregroundStyle(.red)
                        .accessibilityValue(presented.explanation)
                    reasonCodeDetail(presented)
        ...
        // Retry outcome non-success surfacing: a refused or failed retry
        // gets inline feedback instead of looking like the button did
        // nothing. .restarted causes a status reload that makes success
        // visible in the status section; only .refused and .failed need
        // inline feedback.
        if let outcome = model.lastRetryOutcome {
            ...
            case .refused(let reasonCode):
                ...
                VStack(alignment: .leading, spacing: 2) {
                    Label(
                        String(localized: "obsidian.outcome.retry.refused \(presented.explanation)"),
                        systemImage: "hand.raised"
                    )
                    .font(.caption)
                    .foregroundStyle(.red)
                    .accessibilityLabel(
                        String(localized: "obsidian.outcome.retry.refused.a11y \(presented.explanation)")
                    )
                    .accessibilityValue(presented.explanation)
                    reasonCodeDetail(presented)
                }
            case .failed(let reasonCode):
                let presented = CommunityDaemonReason(code: reasonCode)
                VStack(alignment: .leading, spacing: 2) {
                    Text(String(localized: "obsidian.outcome.retry.failed \(presented.explanation)"))
                        .font(.caption)
                        .foregroundStyle(.red)
                        .accessibilityValue(presented.explanation)
                    reasonCodeDetail(presented)
