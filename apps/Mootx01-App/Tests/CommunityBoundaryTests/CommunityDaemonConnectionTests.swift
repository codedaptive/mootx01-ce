...
struct CommunityDaemonConnectionTests {
    @Test("schema-3 descriptor wire format decodes without exposing estate storage")
    ...
    func communityReleaseUsesTeamPrefixedCustodyGroup() throws {
        ...
        let projectURL = try #require(
            ["project.community.yml", "project.yml"]
                .map { appRoot.appendingPathComponent($0) }
                .first { FileManager.default.fileExists(atPath: $0.path) }
        )
        let project = try String(contentsOf: projectURL, encoding: .utf8)
        let targetStart = try #require(project.range(of: "  Mootx01-Community-macOS:"))
        let targetEnd = try #require(
            project.range(
                of: "  Mootx01-Community-UITestHost:",
                range: targetStart.upperBound..<project.endIndex
            )
        )
        let generatedTarget = String(project[targetStart.lowerBound..<targetEnd.lowerBound])

        #expect(groups == ["G94X5T5GK7.group.com.codedaptive.mootx01"])
        #expect(generatedTarget.contains(
