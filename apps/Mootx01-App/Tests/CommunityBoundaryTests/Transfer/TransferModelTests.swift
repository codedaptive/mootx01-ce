...
struct TransferModelTests {
    ...
    @Test("a new import source attempt invalidates the prior plan before cancellation")
    func importReselectionInvalidatesPlan() async {
        let fake = FakeTransferPort()
        let model = TransferModel(port: fake)
        await model.selectImportSource()
        await model.planImport()
        ...
        await fake.setImportSourceOutcome(.cancelled)
        await model.selectImportSource()
        #expect(model.importPlan == nil)
        #expect(!model.canExecuteImport)
    ...
    @Test("a new export destination attempt invalidates the prior plan before cancellation")
    func exportDestinationReselectionInvalidatesPlan() async {
        let fake = FakeTransferPort()
        let model = TransferModel(port: fake)
        await model.selectExportDestination()
        await model.selectExportScope()
        await model.planExport()
        #expect(model.canExecuteExport)

        await fake.setExportDestinationOutcome(.cancelled)
        await model.selectExportDestination()
        #expect(model.exportPlan == nil)
        #expect(!model.canExecuteExport)
    ...
    @Test("a new export scope attempt invalidates the prior plan before cancellation")
    func exportScopeReselectionInvalidatesPlan() async {
        let fake = FakeTransferPort()
        let model = TransferModel(port: fake)
        await model.selectExportDestination()
        await model.selectExportScope()
        ...
        await fake.setExportScopeOutcome(.cancelled)
        await model.selectExportScope()
        #expect(model.exportPlan == nil)
        #expect(!model.canExecuteExport)
