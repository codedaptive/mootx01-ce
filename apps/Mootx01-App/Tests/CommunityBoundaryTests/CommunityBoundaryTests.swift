...
struct CommunityBoundaryTests {
    ...
    func blockedReasonIsVisible() async {
        ...
        // R-C1: the slug is never the status itself — the status is a human
        // sentence, and the exact machine code stays reachable through the
        // labeled, selectable technical detail the shell renders beneath it.
        #expect(!model.status.isEmpty)
        #expect(!model.status.contains(reason))
        #expect(model.statusTechnicalDetail?.contains(reason) == true)
        #expect(model.statusTechnicalDetail != reason,
