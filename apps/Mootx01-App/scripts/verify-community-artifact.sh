...
# The Codedaptive Apple Developer team. PINNED, not derived: every identity
# check below (keychain group, App Group, application-identifier, embedded
# profile team) is built from this constant rather than from the TeamIdentifier
# read out of the candidate bundle. Deriving them from the bundle made the
# checks self-consistent for ANY signer — a counterfeit Community app signed
# and notarized under a different Apple Developer ID satisfied every one of
# them using its own team ID. A team identifier is an identifier, not a
# credential, so pinning it here is safe and is the only thing that makes
# distribution verification a chain-of-custody statement.
readonly CODEDAPTIVE_TEAM="G94X5T5GK7"

distribution=false
# Defaults to the pinned team. --team-id overrides it for a locally signed
# development build under a different account; it can never widen the check,
# because an empty value is refused below.
expected_team="$CODEDAPTIVE_TEAM"
...
while [[ $# -gt 0 ]]; do
  case "$1" in
    ...
    --team-id)
      [[ $# -ge 2 ]] || usage
      # An empty --team-id must not disable the comparison.
      [[ -n "$2" ]] || usage
      expected_team="$2"
      shift 2
      ;;
...
# Unconditional. expected_team is never empty — it defaults to the pinned team
# and --team-id refuses an empty override — so there is no path that reaches
# the identity checks below without having matched a known team first.
...
# Pass expected_team, NOT the team read out of the bundle. The two are equal by
# the check above; using the expectation makes it structurally impossible for a
# future edit to that check to leave these identity comparisons deriving their
# own expectations from the artifact under test.
/usr/bin/python3 - "$info" "$privacy" "$scratch/entitlements.plist" \
  "$scratch/profile.plist" "$expected_team" "$distribution" <<'PY'
import plistlib
import sys
