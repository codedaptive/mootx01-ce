...
//   - moot_list_lenses returns the full cognition menu (18 tools)
...
struct SessionProtocolTests {
    ...
    @Test func teachmeExpandsToNineTierGuide() async throws {
        ...
        #expect(t.contains("44 tools"),
                "nine-tier guide must state the total tool count")
    }

    // MARK: - Test 4: list_lenses returns all 18 cognition tools

    /// `moot_list_lenses` returns the full cognition menu with at least the
    /// 18 Tier 6 tools: `moot_synthesize`, `moot_lens_keystones`, and
    ...
    @Test func listLensesReturnsAllCognitionTools() async throws {
        ...
        #expect(t.contains("18 cognition tools"),
                "moot_list_lenses must report 18 cognition tools")
    }
    ...
    /// at the top; the runtime menu mentions "18 cognition tools".
