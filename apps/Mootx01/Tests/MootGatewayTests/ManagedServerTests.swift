...
//   swift build --package-path apps/ARIA_MCP -c release --product aria-mcp
...
struct ManagedServerTests {
    ...
    private func ariaMCPBinary() -> URL? {
        // …/apps/Mootx01/Tests/MootGatewayTests/ManagedServerTests.swift → repo root is 5 up
        // (file → MootGatewayTests → Tests → Mootx01 → apps → repo root).
        var dir = URL(fileURLWithPath: #filePath)
        for _ in 0..<5 { dir.deleteLastPathComponent() }
        let candidates = [
            dir.appendingPathComponent("apps/ARIA_MCP/.build/release/aria-mcp"),
            dir.appendingPathComponent("apps/ARIA_MCP/.build/debug/aria-mcp"),
        ]
