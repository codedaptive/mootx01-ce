...
fn ingest_turn(
    ...
    // Inline encoding: impatient=true ensures the turn is semantically encoded
    // BEFORE this call returns. Without it, encoding is background — a recall
    // query issued immediately after ingest may run before encoding completes,
    // leading to artificially low recall numbers. This is the LME harness'
    // correctness invariant: ingest → encode → query, with no race.
    // PORT NOTE (1.1.x): parameter renamed from "n" to "impatient" in
    // ToolDispatch.swift/ToolDispatch.rs runFileMemory (D-A inline-encode path).
