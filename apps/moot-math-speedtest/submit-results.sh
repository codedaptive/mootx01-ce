#!/usr/bin/env bash
#
# submit-results.sh — run moot-math-speedtest and bundle the output into a
...
#
# Usage:
#   apps/moot-math-speedtest/submit-results.sh                  # full sweep
#   apps/moot-math-speedtest/submit-results.sh --quick          # smoke run
#   apps/moot-math-speedtest/submit-results.sh --tag MY-RIG     # override hw tag
#   apps/moot-math-speedtest/submit-results.sh --product-binary /path/to/mootx01
#
...
echo "==> moot-math-speedtest bundle: $BUNDLE"
echo "==> $(date)"
...
echo "==> system report"
{
    echo "moot-math-speedtest submission — system report"
...
# moot-math-speedtest submission — ${TAG} — ${DATE}

Hardware tag: \`${TAG}\`
Bundle path:  \`apps/moot-math-speedtest/${BUNDLE}/\`
...
\`\`\`sh
apps/moot-math-speedtest/submit-results.sh --tag ${TAG}$( [ -n "$QUICK" ] && echo " --quick" )
\`\`\`
...
echo "  3. git add apps/moot-math-speedtest/$BUNDLE"
