...
5. The `benchmarks/` gauntlet measures retrieval quality and
...
```
apps/moot-math-speedtest/results/{YYYY-MM-DD}-{hardware-slug}/
  {op}-{language}.json
```
...
under `apps/moot-math-speedtest/results/{date}-{hardware-tag}/`. The
...
Full sweep: apps/moot-math-speedtest/results/2026-05-18-apple-m5-max/
            or_reduce-rust.json
```
