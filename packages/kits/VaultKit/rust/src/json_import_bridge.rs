...
pub struct JsonSeedRecord {
    ...
    /// Schema v1.2: per-record capture timestamp in epoch milliseconds.
    /// When `Some`, the record's capture path uses this as `filed_at`
    /// (and as HLC physical-time seed) instead of the batch `now`.
    /// `None` → batch `now` is used (byte-identical legacy behavior).
    /// Mirrors Swift `JsonSeedRecord.captureDate: Date?`.
...
impl JsonSeedFile {
    ...
    pub fn parse(data: &[u8], limits: &JsonImportLimits, now_ms: i64) -> Result<JsonSeedFile, VaultKitError> {
        ...
        for (index, element) in records_raw.iter().enumerate() {
            records.push(parse_record(element, index, &mut seen_ids, now_ms)?);
        }
...
fn parse_record(
    ...
    if let Some(unknown) = object.keys().find(|k| !RECORD_KEYS.contains(&k.as_str())) {
        return Err(err(format!(
            "{at}: unknown key \"{unknown}\" — schema v1.2 record keys are id, content, event_time, wing, room, kind, sensitivity, exportability, subject, capture_date"
        )));
    }
    ...
    // Schema v1.2 — optional per-record capture timestamp. When present,
    // this record's filed_at (and the HLC physical time derived from it)
    // equals capture_date_ms rather than the batch `now`. Absent → None;
    // capture_batch falls back to the batch wall-clock `now` (byte-identical
    // legacy behavior for all existing imports). Validation mirrors
    // event_time: UTC ISO8601 only; offset forms rejected for cross-port
    // parser parity with the Swift twin.
    let capture_date_ms: Option<i64> = if object.contains_key("capture_date") {
        let raw = object
            .get("capture_date")
            .and_then(|v| v.as_str())
            .ok_or_else(|| {
                err(format!(
                    "{at}: capture_date is not a string (expected UTC ISO8601, e.g. 2026-01-15T10:00:00Z)"
                ))
            })?;
        let ms = parse_utc_iso8601_ms(raw).ok_or_else(|| {
            err(format!(
                "{at}: capture_date is not UTC ISO8601 (\"{raw}\" — expected the form 2026-01-15T10:00:00Z; offset forms are not accepted)"
            ))
        })?;
        // Future-skew gate (codex finding 2026-08-26): capture_date becomes
        // filed_at and the HLC physical time, and the HLC generator advances
        // its clock to any FUTURE value it is handed — one far-future seed
        // row would poison every later write's HLC and the audit timeline
        // with it. Historical dates are the feature and pass untouched;
        // only dates beyond now + 24h (generous zone/skew allowance) are
        // rejected, in total validation so zero-partial-write holds. Twin
        // of the Swift gate.
        if ms - now_ms > 24 * 3600 * 1000 {
            return Err(err(format!(
                "{at}: capture_date \"{raw}\" is more than 24h in the future — a future capture instant would poison the estate's HLC clock; use the real capture time or omit the key"
            )));
        }
        Some(ms)
    } else {
...
pub fn build_frames(file: &JsonSeedFile, default_wing: Option<&str>) -> Vec<CaptureFrame> {
    file.records
        ...
        .map(|record| {
            ...
            // Schema v1.2: per-record capture timestamp. When Some,
            // capture_batch uses this as filed_at rather than the batch now.
            // None → batch now (legacy behavior, byte-identical).
            frame.capture_date = record.capture_date_ms;
            frame
        })
        .collect()
}
...
impl JsonImportBridge<'_> {
    ...
    pub(crate) fn import_seed_windowed(
        ...
        let file = JsonSeedFile::parse(&data, &self.limits, now)?;
...
mod tests {
    ...
    fn valid_fixture_parses_with_defaults_and_file_order() {
        ...
        let file = JsonSeedFile::parse(&data, &JsonImportLimits::default(), 1_790_000_000_000).expect("valid");
    ...
    fn fixture_file() -> JsonSeedFile {
        let data = std::fs::read(fixture_seed_path()).expect("fixture readable");
        JsonSeedFile::parse(&data, &JsonImportLimits::default(), 1_790_000_000_000).expect("valid")
    }
    ...
    // MARK: Part 7 — schema v1.2: capture_date per-record import field
    //
    // Mirrors Swift "JsonImportBridge schema v1.2 — capture_date field" suite.
    // Parser tests are pure (no estate). Pipeline tests use an in-memory estate
    // and assert drawer.filed_at equals the per-record value.

    #[test]
    fn capture_date_parses_to_capture_date_ms() {
        // A valid UTC ISO8601 capture_date populates capture_date_ms on
        // JsonSeedRecord. Parser must accept it without "unknown key" rejection.
        let json = seed(
            r#"[{"id": "cd1", "content": "c", "event_time": "2026-01-01T00:00:00Z",
               "room": "rm", "capture_date": "2026-01-15T10:00:00Z"}]"#,
            "[]",
            "[]",
        );
        let file = JsonSeedFile::parse(json.as_bytes(), &JsonImportLimits::default(), 1_790_000_000_000)
            .expect("parse succeeds");
        let expected_ms =
            parse_utc_iso8601_ms("2026-01-15T10:00:00Z").expect("known-good UTC string");
        assert_eq!(
            file.records[0].capture_date_ms,
            Some(expected_ms),
            "capture_date_ms must equal the parsed UTC instant"
        );
    }

    #[test]
    fn capture_date_absent_is_none() {
        // Records without capture_date must have capture_date_ms == None.
        let json = seed(ONE_RECORD, "[]", "[]");
        let file = JsonSeedFile::parse(json.as_bytes(), &JsonImportLimits::default(), 1_790_000_000_000)
            .expect("parse succeeds");
        assert_eq!(
            file.records[0].capture_date_ms, None,
            "capture_date_ms must be None when capture_date is absent"
        );
    }

    #[test]
    fn malformed_capture_date_is_hard_error() {
        // A non-ISO8601 value must produce an adapterError naming the record.
        expect_parse_error(
            &seed(
                r#"[{"id": "cd-bad", "content": "c", "event_time": "2026-01-01T00:00:00Z",
                   "room": "rm", "capture_date": "not-a-date"}]"#,
                "[]",
                "[]",
            ),
            &JsonImportLimits::default(),
            &["capture_date", "cd-bad"],
        );
    }

    #[test]
    fn offset_capture_date_is_rejected() {
        // Offset forms are not UTC ISO8601 — must be rejected for cross-port parity.
        expect_parse_error(
            &seed(
                r#"[{"id": "cd-offset", "content": "c", "event_time": "2026-01-01T00:00:00Z",
                   "room": "rm", "capture_date": "2026-01-15T10:00:00+05:30"}]"#,
                "[]",
                "[]",
            ),
            &JsonImportLimits::default(),
    ...
    #[test]
    fn capture_date_in_allowed_key_set() {
        // capture_date must not trigger "unknown key" — it is in RECORD_KEYS.
        let json = seed(
            r#"[{"id": "cd-key", "content": "c", "event_time": "2026-01-01T00:00:00Z",
               "room": "rm", "capture_date": "2026-01-15T10:00:00Z"}]"#,
            "[]",
            "[]",
        );
        JsonSeedFile::parse(json.as_bytes(), &JsonImportLimits::default(), 1_790_000_000_000)
            .expect("capture_date is a valid record key — must not produce unknown-key error");
    }

    // Golden pin: fixed capture_date → exact filed_at in BOTH ports.
    //
    // Canonical seed: one record, capture_date "2026-01-15T10:00:00Z".
    // Expected filed_at: 2026-01-15T10:00:00Z
    //   = 1767225600 (2026-01-01 epoch s) + 14*86400 (Jan days) + 10*3600 (hours)
    //   = 1768471200 s = 1768471200000 ms.
    // The Swift twin asserts the same value as expectedMs = 1_768_471_200_000.
    #[test]
    fn golden_pin_capture_date_filed_at() {
        let (mut coordinator, handle) = open_estate();

        // NOW_FAR_FUTURE: a batch-level now far from the capture_date so any
        // accidental fall-through to batchNow produces a clearly wrong filed_at.
        const NOW_FAR_FUTURE: i64 = 2_000_000_000_000i64; // ~2033-05-18

        let json = seed(
            r#"[{"id": "gp1", "content": "Golden pin record.",
               "event_time": "2026-01-15T10:00:00Z", "room": "rm",
        ...
        let drawers = coordinator
            .recall(&handle, frame, NOW_FAR_FUTURE)
            .expect("recall");
        let gp1 = drawers
            .iter()
            .find(|d| d.content == "Golden pin record.")
            .expect("gp1 present");

        // Golden pin: 2026-01-15T10:00:00Z in epoch milliseconds.
        // Derivation: 2026-01-01 = 1767225600 s; +14 days = 1768435200 s;
        // +10 h = 1768471200 s = 1768471200000 ms.
        // Cross-port contract: the Swift twin asserts the same value.
        const EXPECTED_MS: i64 = 1_768_471_200_000i64;
        assert_eq!(
            gp1.filed_at, EXPECTED_MS,
            "golden-pin: filed_at must be 2026-01-15T10:00:00Z (1768471200000 ms); got {}",
            gp1.filed_at
        );
    }
}
