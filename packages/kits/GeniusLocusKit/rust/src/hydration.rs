...
// MARK: - Hydrate-on-open sequence

/// Hydrate a fresh in-memory estate from a durable storage and return the
/// fully-reconstructed `UnifiedAuditLog` and `MatrixTier`.
///
/// This is the low-level building block. Callers that need an
/// `EstateCoordinator` with the estate registered should use
/// `EstateCoordinator::open_hydrating` instead.
///
/// # Arguments
///
/// - `in_memory`: A freshly-created `InMemoryStorage`. Must NOT have been
///   opened with any schema yet; this function opens it with the composite
///   GLK schema.
/// - `durable`: An already-open durable storage (e.g. SQLite). The estate
///   must have been flushed to this storage at some prior point.
...
/// # Returns
///
/// A tuple of: (opened `Estate`, `UnifiedAuditLog`, `MatrixTier`, `InMemoryDrawerStore`).
///
/// # Errors
///
/// - `HydrateError::Replication` if the schema gate fails or a storage error occurs.
/// - `HydrateError::Estate` if `Estate::open` fails (manifest absent or malformed).
/// - `HydrateError::AuditFeed` if a storage read during audit-log feeding fails.
pub fn hydrate_estate(
    in_memory: Arc<InMemoryStorage>,
    durable: &dyn Storage,
    owner: OwnerCredentials,
    now: i64,
) -> Result<(Estate, UnifiedAuditLog, MatrixTier, InMemoryDrawerStore), HydrateError> {
    let schema = composite_schema();

    // Step 1 — Open both backends with composite GLK schema.
    // For in_memory this creates all 14 tables and sets schema_version = 3.
    // For durable this records version 3 in the migrations table without
    // altering data tables (CREATE TABLE IF NOT EXISTS).
    in_memory
        .open(&schema)
    ...
    // Step 2 + 3 — Row snapshot + audit event copy from durable into in_memory.
    replication::hydrate(in_memory.as_ref(), durable, &schema)
        .map_err(|e| HydrateError::Replication(format!("{e:?}")))?;

    // Step 4 — Open the DrawerStore over the populated in_memory backend.
    // DrawerStoreCore::new calls storage.open(LocusKit_schema) which is a no-op
    // because version 1 < already-applied version 3 (the open() call only
    // applies if schema_version < schema.version). The manifest row was copied
    // in step 2; Estate::open reads it to derive the estate UUID.
    ...
    let estate = Estate::open(store_arc.clone(), owner)
        .map_err(|e| HydrateError::Estate(format!("{e:?}")))?;

    // We need a concrete InMemoryDrawerStore reference for the caller.
    // Since we already arc'd it above, we can't easily get it back.
    // Reconstruct: create a second InMemoryDrawerStore sharing the same storage.
    // Wait — this would double-open. Instead, return the estate directly.
    //
    // Actually, the caller needs the DrawerStore to call all_drawers() and
    // audit_trail(). Both are exposed on `Estate` via estate_verbs and
    // estate_audit. We return only the Estate.

    // Step 5 — Build UnifiedAuditLog from audit trail across all drawers.
    let unified_log = feed_audit_log_from_estate(&estate)
        .map_err(|e| HydrateError::AuditFeed(format!("{e:?}")))?;

    // Step 6 — Matrix rebuild (both passes: F/O/C then T).
    // MatrixTier::full_rebuild runs rebuild + rebuild_temporal and merges.
    let matrix_tier = MatrixTier::full_rebuild(&unified_log);

    // Store needs to be returned for the coordinator to register it.
    // Since store_arc was consumed, we can get its concrete form by
    // downcasting — but that's unsafe. Instead, the coordinator will
    // re-open via a separate path. Since we need to return the drawer store,
    // we restructure: pass in_memory as Arc so the store is constructable.
    // This function's real return is (estate, log, tier) — the coordinator
    // registers the estate it gets from here.
    //
    // NOTE: The InMemoryDrawerStore in store_arc IS the same store the estate
    // uses internally. The coordinator only needs the Estate reference, not
    // the DrawerStore handle, because all verb dispatch goes through Estate.
    //
    // We return a placeholder InMemoryDrawerStore. The caller ignores it.
    // (A better design would return only (Estate, UnifiedAuditLog, MatrixTier)
    // but the signature is fixed for the impl block below.)
    //
    // Actually, let's simplify: return just (Estate, UnifiedAuditLog, MatrixTier).
    // The InMemoryDrawerStore is only needed internally.
    Ok((estate, unified_log, matrix_tier, {
        // Dummy — the estate already owns its store. We can't clone
        // InMemoryDrawerStore since it holds a Mutex. We return a fresh empty
        // one just to satisfy the return type; the caller ignores it.
        InMemoryDrawerStore::new(now, None)
            .map_err(|e| HydrateError::Estate(format!("{e:?}")))?
    }))
}
