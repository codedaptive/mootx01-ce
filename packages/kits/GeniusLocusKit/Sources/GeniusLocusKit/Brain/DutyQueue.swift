...
// the queue on its ordinary cadence; `payDutyUntilSettled` is the settle
// loop, the bulk path (`mootx01 drain`, `mootx01 upgrade`). "Impatient" names
// only the single-record write mode, never a duty (§ DUTY_LIFECYCLE).
//
...
public extension GeniusLocusKit {
    ...
    // Batch sizes per job come from `dutyLimits(for:)` (DutyLimits.swift),
    // host-supplied from the settings module.
    ...
    /// batch pays nothing. Returns the units paid in total. `progress` sees
    /// every batch that ran, so a finisher can report each one.
    @discardableResult
    func payDutyUntilSettled(
        _ kind: DutyKind, in handle: EstateHandle, now: Date,
    ...
    private func runDutyBatch(_ kind: DutyKind, in handle: EstateHandle, now: Date) async throws -> Int {
        ...
        case .subjectBackfill:
            guard subjectProducers[handle] != nil else { return 0 }
            return try await subjectBackfillSweep(handle, batchLimit: dutyLimits(for: handle).subjectBackfillBatch, now: now).written
        case .factsBackfill:
            return try await backfillSSCFacts(handle: handle)
        case .factExtraction:
            return try await runFactExtractionBatch(handle, limit: dutyLimits(for: handle).factExtractionBatch, now: now).completedSources
        case .retrainBasis:
