...
// catalog chain continues through the 1.2→1.3 capsule, so every prepared
// estate ends at the current format, v1_3.
//
// Tests:
//   1. v1_1-stamped estate with pre-v3 (version 2) schema: after prepare the
//      column exists, the estate is stamped current (v1_3), and a row can be
...
//   4. v1_0-stamped estate: the full chain (v1_0→v1_1→v1_2→v1_3) ends at v1_3.
...
struct IndexCompositionColumnMigrationTests {
    ...
    /// current (v1_3), and a store-API write round-trips a compositionPolicyID.
    @Test
    ...
    // MARK: §4 Full chain from v1_0 ends at v1_3

    #if GLK_MIGRATION_V1_0_TO_V1_1
    /// A v1_0-stamped estate (no legacy chunks) runs the full catalog chain
    /// (v1_0→v1_1→v1_2→v1_3) and ends at v1_3. Uses the simplest possible
    ...
    func v1_0EstateRunsFullChainToV1_3() async throws {
        ...
        // stamps v1_2; the v1_2→v1_3 capsule stamps v1_3. migrated=false
        // because no legacy chunk data was moved.
        #expect(prep.format == .v1_3)
        #expect(prep.migrated == false)
