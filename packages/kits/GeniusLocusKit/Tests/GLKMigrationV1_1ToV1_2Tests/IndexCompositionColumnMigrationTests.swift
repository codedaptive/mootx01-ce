...
// estates created before corpus_index_state reached schema version 3.
//
// Tests:
//   1. v1_1-stamped estate with pre-v3 (version 2) schema: after prepare the
//      column exists, the estate is stamped v1_2, and a row can be written
//      and read back with a non-empty compositionPolicyID.
//   2. Idempotence: a second prepare on the already-migrated estate is a no-op
//      that leaves the stamp at v1_2.
//   3. Fresh estate (nil stamp): prepare stamps v1_2 without running any capsule.
//   4. v1_0-stamped estate: the full chain (v1_0→v1_1→v1_2) ends at v1_2.
...
struct IndexCompositionColumnMigrationTests {
    ...
    /// v1_2, and a store-API write round-trips a compositionPolicyID.
    @Test
    ...
    func prepareTwiceIsNoOp() async throws {
        ...
        #expect(second.format == .v1_2)
        #expect(second.migrated == false)
    }

    // MARK: §3 Fresh estate (nil stamp): stamped v1_2 without running capsule
    ...
    // MARK: §4 Full chain from v1_0 ends at v1_2

    #if GLK_MIGRATION_V1_0_TO_V1_1
    /// A v1_0-stamped estate (no legacy chunks) runs the full catalog chain
    /// (v1_0→v1_1→v1_2) and ends at v1_2. Uses the simplest possible fixture
    /// to avoid duplicating the SharedContentMigration legacy-estate builder.
    @Test
    func v1_0EstateRunsFullChainToV1_2() async throws {
        ...
        // estate (no legacy chunks) and stamps v1_1; then the v1_1→v1_2 capsule
        // stamps v1_2. migrated=false because no legacy chunk data was moved.
        #expect(prep.format == .v1_2)
        #expect(prep.migrated == false)
