...
//      prepare the LocusKit kit version is 18, the estate is stamped v1_3, and
...
//   3. Fresh estate (nil stamp): prepare stamps v1_3 without running any capsule.
//   4. v1_1-stamped estate: the 1.1→1.2 and 1.2→1.3 capsules run and end at v1_3.
//   5. v1_0-stamped estate: the full chain (v1_0→v1_1→v1_2→v1_3) ends at v1_3.
...
struct DistilledSourceDigestColumnMigrationTests {
    ...
    /// is v1_3, and a representation write round-trips its digest.
    @Test
    ...
    func prepareTwiceIsNoOp() async throws {
        ...
        #expect(second.format == .v1_3)
        #expect(second.migrated == false)
    }

    // MARK: §3 Fresh estate (nil stamp): stamped v1_3 without running capsule
    ...
    // MARK: §4 v1_1 estate runs 1.1→1.2 then 1.2→1.3

    #if GLK_MIGRATION_V1_1_TO_V1_2
    @Test
    func v1_1EstateRunsTwoCapsulesToV1_3() async throws {
    ...
    // MARK: §5 Full chain from v1_0 ends at v1_3

    #if GLK_MIGRATION_V1_0_TO_V1_1
    /// A v1_0-stamped estate (no legacy chunks) runs the full catalog chain
    /// (v1_0→v1_1→v1_2→v1_3) and ends at v1_3.
    @Test
    func v1_0EstateRunsFullChainToV1_3() async throws {
        ...
        // the 1.2→1.3 capsule stamps v1_3. migrated=false because no legacy
        // chunk data was moved.
        #expect(prep.format == .v1_3)
        #expect(prep.migrated == false)
        #expect(try await EstateFormatStore(storage: storage).readIfPresent() == .v1_3)
        let content = "Digest column round-trip after the 1.2 to 1.3 capsule."
