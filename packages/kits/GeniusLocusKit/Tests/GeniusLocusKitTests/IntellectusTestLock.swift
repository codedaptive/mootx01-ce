...
//
// Process-wide mutex serialising every GeniusLocusKit test that either:
//   (a) touches the Intellectus global singleton (install/setEnabled), or
//   (b) calls a GLK method that crosses a telemetry emit site (open, close,
//       provision, quiesce, drain, and the nine verb remap boundaries).
//
...
// and enables monitoring, running concurrently with a CoordinatorLifecycle
// or VerbSurface test that emits into that same sink, corrupts the
// telemetry test's exact-count assertions — producing intermittent
// failures under the default parallel runner.
//
// SOLUTION (mirrors NeuronKit, SynapseKit)
// This is the Swift equivalent of the `GLOBAL_LOCK: Mutex<()>` held by
// every test in glk_telemetry_tests.rs. A disabled-path test that runs
// lock-free can interleave with a lock-held enabled-path test and corrupt
// it — so the lock is unconditional on all emit-crossing callers.
//
...
// functions that touch the singleton or call emitting GLK methods.
//
...
/// Serialises all GeniusLocusKit tests that share the Intellectus singleton.
