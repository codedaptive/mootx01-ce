...
/// `vectors(forDrawerID:)` round-trips; this suite covers the value
/// type's own contract directly: memberwise initialization retains
/// every field, and `Equatable` is field-wise across all six members
/// (`id`, `drawerID`, `modelID`, `modelVersion`, `engram`, `filedAt`).
@Suite("StoredVector")
struct StoredVectorTests {

    private func sample(id: String = "row-1",
                        drawerID: String = "drawer-A",
    ...
    -> StoredVector {
        StoredVector(id: id, drawerID: drawerID, modelID: modelID,
                     modelVersion: modelVersion, engram: engram, filedAt: filedAt)
    }
    ...
    @Test func testInequalityWhenAnyFieldDiffers() {
        ...
        #expect(base != sample(drawerID: "drawer-B"))
