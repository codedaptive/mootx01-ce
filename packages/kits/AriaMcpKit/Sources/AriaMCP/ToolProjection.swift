    ...
    static func estateTools() -> [ProjectedTool] {
        [
            ...
            ProjectedTool(
                ...
                inputSchema: withEstateID(objectSchema(
                    properties: [
                        "since_ms": integerSchema("Optional watermark from a previous call's watermark_ms (epoch milliseconds). Only captures strictly after it are measured, so successive calls never double-count. Omit or 0 to scan from the beginning (clamped at 262144 events per call; page forward via watermark_ms)."),
                    ],
                    required: []
                )),
                provenance: .interface
