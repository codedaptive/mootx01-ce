...
impl V2RetireFactRequest {
    pub fn decode(v: &JsonValue) -> V2DecodeResult<Self> {
        let o=strict_object(v,["fact_id","reason","estate_id"])?;
        Ok(Self {
            fact_id: required_uuid(o,"fact_id")?,
            // optional_nonempty checks for empty string only; no length cap applies to reason.
            reason: optional_nonempty(o,"reason")?,
...
impl V2KnowledgeJournalLower for CoordinatorKnowledgeJournalLower {
    ...
    // caller_binding is the server identity injected at registry construction time;
    // reason is forwarded from the caller's request (optional_nonempty, no length cap).
    fn retire_fact(&self,a:&V2KnowledgeJournalAdmission,r:&V2RetireFactRequest)->Result<(),()>{self.coordinator.lock().map_err(|_|())?.withdraw_kg_fact(&a.estate_handle,&r.fact_id.to_string(),&a.caller_binding,r.reason.as_deref(),a.now_millis).map_err(|_|())}
