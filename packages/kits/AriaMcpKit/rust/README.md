...
server at `packages/kits/AriaMcpKit/Sources/AriaMCP/`.
...
and its own MCP server. The Swift vertical is whole (packages/kits/AriaMcpKit). Until this
server shipped, the Rust vertical's kits executed only under `cargo test` — a
tracked shortfall, not doctrine.

## Running

This crate is the server-logic **library**; the runnable `aria-mcp` binary lives
in `apps/aria-mcp-server/rust` and links this lib.

```sh
cargo run --manifest-path apps/aria-mcp-server/rust/Cargo.toml
```

Or from `apps/aria-mcp-server/rust`:
