...
//! all three backends: SQLite, in-memory, and PostgreSQL.
//!
//! # What these tests prove
//!
//! All three backends now wire semantic recall — `Corpus` + `VectorStore` are
//! registered after `coord.open` for every backend. `moot_memory_search` uses
//! BM25 + vector lanes on every backend from the first capture.
//!
...
//! - PostgreSQL (ARIA_MCP_POSTGRES_URL set): semantic recall wired via a
//!   `PostgresStorage` handle on the same connection string.
//!
//! # Test coverage
//!
...
/// exist yet — `new_sqlite` creates it. Caller removes the file on teardown.
fn temp_sqlite_path(label: &str) -> String {
    let name = format!("aria_mcp_semantic_{}_{}.sqlite", label, Uuid::new_v4());
    std::env::temp_dir()
        .join(name)
        .to_string_lossy()
        .into_owned()
...
fn sqlite_semantic_lanes_lit_after_register_sqlite() {
    ...
    // Capture into the additional estate using impatient mode.
    let capture_args = args![
        "content" => "the egret stalks through marshland",
        "location" => "memories/birds",
        "impatient" => true,
        "estateID" => extra_id.to_string(),
    ];
    let capture_result = dispatch_tool("moot_file_memory", &capture_args, &registry, &ledger)
        .expect("moot_file_memory on extra estate must not fail");
    assert!(
        is_success(&capture_result),
        "impatient moot_file_memory on register_sqlite estate should succeed; got: {capture_result:?}"
    );

    // Search the additional estate by estateID.
    let search_args = args![
        "query" => "egret marshland",
        "scoring" => "rrf",
        "estateID" => extra_id.to_string(),
    ];
    let search_result = dispatch_tool("moot_memory_search", &search_args, &registry, &ledger)
        .expect("moot_memory_search on extra estate must not fail");
    assert!(
        is_success(&search_result),
        "moot_memory_search on register_sqlite estate should succeed; got: {search_result:?}"
    );

    let text = content_text(&search_result);
    assert!(
        text.starts_with("found ") && !text.starts_with("found 0"),
        "expected at least 1 result on register_sqlite estate; got: {text}"
    );
    assert!(
        text.contains("egret"),
        "search result should contain captured content; got: {text}"
