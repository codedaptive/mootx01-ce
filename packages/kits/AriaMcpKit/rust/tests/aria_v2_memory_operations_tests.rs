...
impl V2CoreMemoryService for Fake {
    ...
    fn search_memories(&self, _: &V2MemoryOperationContext, request: &V2MemorySearchRequest) -> Result<V2MemorySearchResult, V2MemoryFailure> { self.searches.lock().unwrap().push(request.clone()); Ok(V2MemorySearchResult { rows: (0..3).map(|_| V2CompactMemory { memory_id: Uuid::parse_str("A0B1C2D3-E4F5-4678-9012-3456789ABCDE").unwrap(), subject: Some("retrieved".into()), score: Some(0.8), provenance: None, context: Some("😀".repeat(513)), excerpt: Some("🦀".repeat(513)), fetch: V2FetchReference { tool: MEMORY_GET_TOOL, arguments: V2FetchArguments { memory_id: "ignored".into() } } }).collect(), answer_block: None, degraded: false }) }
