...
// firstSentence leaked a body-derived preview for exactly the rows whose
// subject said they were redacted. The Rust twin nils first_sentence for
...
struct FederatedRedactionTests {
    ...
    @Test("restricted row redacts subject AND drops firstSentence")
    func restrictedRedactsBothColumns() {
        ...
        #expect(row.firstSentence == nil)
    }

    @Test("secret row redacts subject AND drops firstSentence")
    ...
    func normalKeepsBothColumns() {
        for sensitivity in [Sensitivity.normal, .elevated] {
            ...
            #expect(row.firstSentence == "Flight leaves at nine.")
        }
    }
