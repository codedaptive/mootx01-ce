...
//
// The gate is LocusKit's default sensitivity ceiling, which
// `BitmapEvaluator.insertDefaults` turns into `.sensitivityAtMost(.elevated)`
// on the adjective axis for every frame that names no sensitivity predicate.
// The lens arms below hydrate their endpoints through that gate.
