...
struct FdcCaptureTests {
    ...
    /// allowing historical article terms to select an unrelated code.
    ///
    ...
    ///
    /// The shared FDC fixture pins both phrases to code 004.
    @Test func reviewedComputingAliasesSurviveCapture() async throws {
        let (dispatcher, kit, handle) = try await makeDispatcher()
        ...
        for drawer in drawers {
            ...
            #expect(
                udc == "004",
                "reviewed computing aliases must survive capture; got: '\(udc)'"
            )
        }
    }
