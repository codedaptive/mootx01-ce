// SSCFactsColumnCryptoTests.swift
//
// SPEC_DISTILLATION_STORAGE §2 — "At-rest protection is inherited":
// the `ssc_facts` column is content-derived SSC fact text and carries
// the same row-level protection class as `content` under the Mode-2
// row-crypto seam. These tests pin:
//
//   1. `encryptedForWrite` seals `ssc_facts` exactly as it seals
//      `content` — including on a representation-only value map (an
//      SSC-facts write is an UPDATE that carries no `content`).
//   2. `decryptedForRead` opens a sealed `ssc_facts` back to text.
//   3. `assertContentKeyIDInvariant` refuses plaintext `ssc_facts`
...
//      the seam, so an SSC-facts UPDATE round-trips through storage
//      as ciphertext at rest and text on read.
...
struct SSCFactsColumnCryptoTests {
    ...
    private func freshDBURL() -> URL {
        let dir = FileManager.default.temporaryDirectory
            .appendingPathComponent("ssc-facts-crypto-\(UUID().uuidString)", isDirectory: true)
    ...
    @Test("encryptedForWrite seals ssc_facts alongside content and stamps keyID")
    func sealsSSCFactsWithContent() throws {
        let config = EstateEncryptionConfig(.rowEncryption)
        let values: [String: TypedValue] = [
            "id": .text("d1"),
            "content": .text("original body"),
            "ssc_facts": .text("kind: hobby, entity: painting"),
        ]
        ...
        guard case .blob = sealed["ssc_facts"] else {
            Issue.record("ssc_facts was not sealed"); return
        }
    ...
    func sealsRepresentationOnlyWrite() throws {
        ...
        let values: [String: TypedValue] = ["ssc_facts": .text("kind: hobby, entity: painting")]
        let sealed = try encryptedForWrite(values, table: "drawers", config: config)
        guard case .blob = sealed["ssc_facts"] else {
            Issue.record("representation-only ssc_facts was not sealed"); return
        }
        #expect(sealed["keyID"] == .text(config.keyIdentifier!))
    }

    @Test("decryptedForRead opens a sealed ssc_facts back to text")
    func opensSSCFacts() throws {
        let config = EstateEncryptionConfig(.rowEncryption)
        let sealed = try encryptedForWrite(
            ["content": .text("body"), "ssc_facts": .text("kind: hobby, entity: painting")],
        ...
        #expect(opened["ssc_facts"] == .text("kind: hobby, entity: painting"))
    }

    @Test("plaintext estates pass ssc_facts through unchanged (Mode 1 no-op)")
    func plaintextNoOp() throws {
        ...
        #expect(out["ssc_facts"] == .text("kind: hobby, entity: painting"))
        #expect(out["keyID"] == nil)
    }

    @Test("invariant guard refuses plaintext ssc_facts with no keyID on an encrypting estate")
    func invariantRefusesPlaintextSSCFacts() throws {
        let config = EstateEncryptionConfig(.rowEncryption)
        #expect(throws: (any Error).self) {
            try assertContentKeyIDInvariant(
                ["id": .text("d1"), "ssc_facts": .text("kind: location, entity: Paris")],
                table: "drawers",
                config: config)
        }
    ...
    @Test("SSC-facts UPDATE on an encrypting estate round-trips (sealed at rest, text on read)")
    func updateRowsSealsSSCFacts() async throws {
        let config = EstateEncryptionConfig(.rowEncryption)
        let url = freshDBURL()
        ...
        // The SSC-facts write: representation-only UPDATE.
        let updated = try await storage.rowStore.update(
            table: "drawers",
            values: ["ssc_facts": .text("kind: hobby, entity: painting")],
        ...
        let rows = try await storage.rowStore.query(
            table: "drawers",
            where: .eq(Column(table: "drawers", name: "id"), .text("d1")))
        #expect(rows.first?["ssc_facts"] == .text("kind: hobby, entity: painting"))
        ...
        let rawRows = try await rawStorage.rowStore.query(
            table: "drawers",
            where: .eq(Column(table: "drawers", name: "id"), .text("d1")))
        guard case .blob = rawRows.first?["ssc_facts"] else {
            Issue.record("ssc_facts stored as plaintext at rest after updateRows"); return
        }
    ...
    @Test("encryptedForWrite seals subject alongside content and ssc_facts")
    ...
    @Test("capture-time write seals content, ssc_facts, and subject")
    ...
    func updateRowsErasureStillPasses() async throws {
        ...
        _ = try await storage.rowStore.insert(
            table: "drawers",
            values: ["id": .text("d1"), "content": .text("body"), "ssc_facts": .text("kind: hobby, entity: painting")])

        // The expunge scrub shape: zero content, NULL the representation.
        let updated = try await storage.rowStore.update(
            table: "drawers",
            values: ["content": .text(""), "ssc_facts": .null],
            where: .eq(Column(table: "drawers", name: "id"), .text("d1")))
        #expect(updated == 1)
        ...
        // exemption); ssc_facts is gone.
        #expect(rows.first?["content"] == .text(""))
        #expect(rows.first?["ssc_facts"] == nil || rows.first?["ssc_facts"] == .null)
    }
}
...
struct ProtectedColumnMapCoverageTests {
    ...
    /// drop `ssc_facts` or `subject` from the map — the shape the mission
