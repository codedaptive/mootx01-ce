// row_key_determinism_inmemory_tests.rs
//
// Gap 5 fix verification — InMemoryRowStore::resolve_key.
//
// Rust twin of PersistenceKitInMemoryTests/RowKeyDeterminismMoneyTests.swift.
// See that file's header for the full gap-5 writeup. THE MONEY TEST: two
// INDEPENDENT storage instances (the direct model of two federation spokes)
// writing a row with the SAME single-column `.text` primary-key VALUE must
// resolve to the SAME internal `RowKey`.

use persistence_kit::{
    inmemory::InMemoryStorage, ColumnDeclaration, SchemaDeclaration, Storage, TableDeclaration,
    TypedValue,
};
use std::collections::BTreeMap;
use uuid::Uuid;

fn make_widgets_storage() -> InMemoryStorage {
...
#[test]
fn same_uuid_shaped_text_pk_resolves_same_key_across_instances() {
...
/// COMPLETELY broken before gap 5 (not even a UUID-string-parse fallback
/// existed on `InMemoryRowStore::resolve_key`).
#[test]
fn same_non_uuid_text_pk_resolves_same_key_across_instances() {
    let storage_a = make_widgets_storage();
    let storage_b = make_widgets_storage();
    ...
    // Cross-check against the shared vector (row_key_derivation.rs,
    // RowKeyDerivationConformanceTests.swift, MerkleRollupTests.swift).
    assert_eq!(
