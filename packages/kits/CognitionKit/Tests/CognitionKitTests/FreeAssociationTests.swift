...
/// Free association — structure lens (category 1). Spreads activation from
/// a seed drawer over a wing's drawer-to-drawer tunnel graph (a restart
/// random walk, NeuronKit spreadingActivation) and returns the top-k
/// most-activated memories. Read-only; "what this memory reminds me of."
...
struct FreeAssociationTests {
    ...
    // direct neighbour outranks the two-hop memory, the seed is excluded, and
    // a disconnected component never surfaces.
    @Test("associates reachable, excludes unreachable")
    func associatesReachableExcludesUnreachable() async throws {
        let (kit, handle) = try await openEstate()
        // Reachable from S: S<->A, A<->C.
    ...
    // CK-FA-2: a seed absent from the graph yields no associations.
