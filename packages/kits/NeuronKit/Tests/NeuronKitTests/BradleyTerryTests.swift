...
//
// ISOLATION NOTE
// Every test is declared `async` and acquires the process-wide
// intellectusTestMutex via withIntellectusLock (IntellectusTestLock.swift)
// because bradleyTerry() emits to the global Intellectus singleton.
// A concurrent telemetry test that has enabled monitoring and installed a
// capturing sink would see extra emissions from this suite, corrupting
// its exact-count assertions. The async mutex serialises all
// singleton-touching tests across suite boundaries without blocking any
// thread (uses cooperative suspension via CheckedContinuation).
...
struct BradleyTerryTests {
    ...
    func sameInputsProduceBitForBitIdenticalScores() async throws {
    ...
    func rankingIsInvariantToInputOrder() async throws {
        try await withIntellectusLock {
    ...
    func transitiveDominanceRanksAOverBOverC() async throws {
        try await withIntellectusLock {
    ...
    func everyScoreHasFiniteConfidenceIntervalBracketingStrength() async throws {
        try await withIntellectusLock {
            let scores = try bradleyTerry(outcomes: dominanceLadder)
            for score in scores {
    ...
    func symmetricOutcomesProduceEqualStrengthsAndOverlappingCIs() async throws {
        try await withIntellectusLock {
    ...
    func countAggregationEqualsRepeatedSingleOutcomes() async throws {
        try await withIntellectusLock {
            let aggregated = [
            ...
            // Separate the two try-calls to avoid try-inside-#expect.
            let scoresAggregated = try bradleyTerry(outcomes: aggregated)
            let scoresExpanded = try bradleyTerry(outcomes: expanded)
            #expect(scoresAggregated == scoresExpanded)
        }
    }

    @Test("non-positive count contributes nothing")
    func nonPositiveCountContributesNothing() async throws {
        try await withIntellectusLock {
            let withZero = dominanceLadder + [
                PairwiseOutcome(winner: "A", loser: "B", count: 0),
                PairwiseOutcome(winner: "C", loser: "B", count: -4),
            ]
            // Separate try-calls to avoid try-inside-#expect.
            let baselineScores = try bradleyTerry(outcomes: dominanceLadder)
            let withZeroScores = try bradleyTerry(outcomes: withZero)
            #expect(baselineScores == withZeroScores)
        }
    }
    ...
    func selfPairingThrows() async throws {
        try await withIntellectusLock {
    ...
    func emptyInputReturnsEmpty() async throws {
        try await withIntellectusLock {
            // Use a let binding to avoid try-inside-#expect.
            let result = try bradleyTerry(outcomes: [])
            #expect(result == [])
        }
    }
    ...
    func convergesToStationaryPointSatisfyingMMCondition() async throws {
        try await withIntellectusLock {
    ...
    func disconnectedComponentsThrow() async throws {
        try await withIntellectusLock {
    ...
    func pureTransitiveIsNotFinite() async throws {
        try await withIntellectusLock {
