...
final class DreamingDaemonTests: XCTestCase {

    // MARK: - C-1: tick cadence (injectable clock, no wall-clock sleeps)

    func testC1_TickFiresOnConfiguredIntervalWithinTolerance() async throws {
        ...
        XCTAssertNil(early, "tick must not fire before the interval elapses")

        // At the interval: fires.
        let second = try await d.pump(now: t0.addingTimeInterval(30))
        let secondReport = try XCTUnwrap(second)
        ...
        XCTAssertGreaterThanOrEqual(spacingMs, 27_000)
        XCTAssertLessThanOrEqual(spacingMs, 33_000)
    }

    // MARK: - C-2: confidence (and attempts) gate

    func testC2_ProposesOnlyAboveConfidenceAndAttempts() async throws {
        ...
        XCTAssertEqual(report.proposalsEmitted.count, 1, "only the strong candidate clears both gates")
        XCTAssertEqual(report.belowThreshold, 2, "low-confidence and few-attempts candidates are gated out")
        XCTAssertEqual(report.proposalsEmitted.first?.target, "a")
        XCTAssertEqual(report.proposalsEmitted.first?.kind, .miningPattern)
    }

    // MARK: - C-6: exactly one diary entry per cycle

    func testC6_OneDiaryEntryPerCycle() async throws {
        let reader = FakeReader()
        let sink = RecordingSink()
        let d = daemon(reader: reader, sink: sink)
    ...
    func testC15_RewardDerivedFromRecallTraceUsed() async throws {
        let reader = FakeReader(
        ...
        XCTAssertEqual(report.rewardByTarget["r1"], 1.0)
        XCTAssertEqual(report.rewardByTarget["r2"], 0.0)
        ...
        XCTAssertEqual(kind, .recallTrace)
    }
    ...
    func testB4_SecondCycleOverUnchangedStateProposesNothingNew() async throws {
        ...
        XCTAssertEqual(first.proposalsEmitted.count, 1)

        let second = try await d.triggerDreamingCycle(now: t0.addingTimeInterval(60))
        XCTAssertEqual(second.proposalsEmitted.count, 0, "already-proposed candidate is suppressed")
        XCTAssertGreaterThanOrEqual(second.suppressedDuplicates, 1)
    ...
    func testInvariant_NeverCreatesTunnelOnlyProposes() async throws {
        let reader = FakeReader(
            traces: [trace("r1", used: true), trace("r2", used: true)],
            observations: [
        ...
        for frame in report.proposalsEmitted {
            XCTAssertEqual(frame.kind, .miningPattern, "emissions are proposals, not tunnels")
        }
        let diaryCount = await sink.diaryCount()
        XCTAssertEqual(diaryCount, 1)
    }

    // MARK: - Step 5: existing Tunnel suppresses a duplicate candidate

    func testDuplicateOfExistingTunnelIsSuppressed() async throws {
        let existing = Tunnel(
            id: "tun-1",
        ...
        let report = try await d.triggerDreamingCycle(now: t0)
        XCTAssertEqual(report.proposalsEmitted.count, 0, "candidate duplicates an existing Tunnel")
        XCTAssertGreaterThanOrEqual(report.suppressedDuplicates, 1)
    }

    // MARK: - Step 4: EWC++ does not catastrophically overwrite priors

    func testEWC_PriorAssociationNotCatastrophicallyOverwritten() async throws {
        let reader = FakeReader(
            traces: [trace("r1", used: true), trace("r2", used: true)],
            observations: [
                CoOccurrenceObservation(endpointA: "a", endpointB: "b", attempts: 3, evidenceTargets: ["r1", "r2"]),
        ...
        let firstScore = try XCTUnwrap(first.candidateScores[key])
        XCTAssertGreaterThanOrEqual(firstScore, 0.7)
        ...
        let second = try await d.triggerDreamingCycle(now: t0.addingTimeInterval(60))
        let secondScore = try XCTUnwrap(second.candidateScores[key])
        ...
        XCTAssertLessThan(freshRaw, 0.1, "fresh score collapsed without retention")
        XCTAssertGreaterThanOrEqual(secondScore, 0.7, "prior association preserved by EWC++ retention")
        XCTAssertGreaterThan(secondScore, freshRaw)
    }
    ...
    func testPolicyRoundTripsThroughManifestSeam() async throws {
        ...
        XCTAssertEqual(loaded, DreamingPolicy(
            minSuccessRate: 0.5, minConfidence: 0.8, minAttempts: 5, tickIntervalMs: 10_000
        ))
    }
