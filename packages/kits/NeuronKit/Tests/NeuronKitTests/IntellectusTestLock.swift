...
//
// Process-wide mutex serialising every NeuronKit test that either:
//   (a) touches the Intellectus global singleton (install/setEnabled), or
//   (b) calls a telemetry-emitting function (bradleyTerry,
//       HybridRecallEngine.rerank, DreamingDaemon.triggerDreamingCycle).
//
...
// and enables monitoring, running concurrently with a BradleyTerry or
// DreamingDaemon test that emits into that same sink, corrupts the
// telemetry test's exact-count assertions — producing intermittent
// failures under the default parallel runner.
//
// SOLUTION (mirrors Rust)
// This is the Swift equivalent of the `GLOBAL_LOCK: Mutex<()>` held by
// every test in neuronkit_telemetry_tests.rs, including disabled-path
...
/// Serialises all NeuronKit tests that share the Intellectus singleton.
