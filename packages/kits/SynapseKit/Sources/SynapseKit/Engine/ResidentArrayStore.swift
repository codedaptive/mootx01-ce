...
public actor ResidentArrayStore {
    ...
    static func parseSidecar(_ data: Data) throws -> ResidentVectorArray {
        ...
        guard data.count >= 4 else {
            throw SynapseKitError.decodingFailure("ResidentArrayStore: sidecar too short for magic")
        }
        let magic = Array(data[0..<4])
        guard magic == kVecMagic else {
            throw SynapseKitError.decodingFailure(
        ...
        guard data.count >= offset + 2 else {
            throw SynapseKitError.decodingFailure("ResidentArrayStore: truncated at version field")
        }
        ...
        guard offset < data.count else {
            throw SynapseKitError.decodingFailure("ResidentArrayStore: truncated at kind")
        }
        ...
        guard data.count >= offset + 4 else {
            throw SynapseKitError.decodingFailure("ResidentArrayStore: truncated at stride field")
        }
        let stride = data.readLE32(at: offset); offset += 4

        // --- Count ---
        guard data.count >= offset + 4 else {
            throw SynapseKitError.decodingFailure("ResidentArrayStore: truncated at count field")
        }
        ...
        // --- Tombstones ---
        guard data.count >= offset + 4 else {
            throw SynapseKitError.decodingFailure("ResidentArrayStore: truncated at tombstone_words field")
        }
        ...
        guard !tsOverflow else {
            throw SynapseKitError.decodingFailure("ResidentArrayStore: tombstone block size overflows")
        }
        tombstoneBlockBytes = tsBytes
        guard offset + tombstoneBlockBytes <= data.count else {
            throw SynapseKitError.decodingFailure("ResidentArrayStore: truncated in tombstone block")
        }
        ...
        guard !vbOverflow else {
            throw SynapseKitError.decodingFailure("ResidentArrayStore: vectors block size overflows")
        }
        vectorsBytes = vb
        guard offset + vectorsBytes <= data.count else {
            throw SynapseKitError.decodingFailure("ResidentArrayStore: truncated in vectors block")
        }
