...
//
// Metrics: cosine, L2, dot — via DenseMetric.float(FloatMetric), SynapseKit's
// own float-lane metric type (SynapseKit-owned vector search persistencekit-vector-contract-correction).
//
...
/// error and throws SynapseKitError.invalidPayload.
///
...
public actor FloatBruteForceIndex: DenseIndex {
    ...
    ///
    /// Throws SynapseKitError.invalidPayload if:
    ...
    ///
    /// Throws the same `SynapseKitError.invalidPayload` cases as `search`.
    ...
    /// slots are preserved.
    ///
    /// Throws `SynapseKitError.invalidPayload` if the vector kind is not float32
