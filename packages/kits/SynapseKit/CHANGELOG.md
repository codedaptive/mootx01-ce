# SynapseKit Changelog

## 2026-09-04 — SYN-CODE-2: VectorKit is SynapseKit

**Branch:** `rename/synapsekit`

The kit is renamed. The old name collided with Apple's MapKit VectorKit
framework. Both ports, the build graph, and every identifier carrying the
kit name changed: the Swift package, product, and module are `SynapseKit`;
the Rust crate is `synapsekit`; `VectorKitError` is `SynapseKitError`; the
OSLog category, the telemetry metric namespace (`synapsekit.*`) and its
`kit` tag, and the two stored schema-version ledger ids (`SynapseKit`,
`SynapseKitClaims`) follow. The store types (`VectorStore`,
`VectorRepresentationClaims`, `VectorPayload`, ...) keep their names: they
name what they hold, not the kit.

Populated estates carry the two ledger ids as stored rows. GeniusLocusKit's
1.4 → 1.5 migration capsule (`StorageLedgerKitIDMigration`, estate format
V1_5) moves them through PersistenceKit's new `renameSchemaKit(from:to:)`
before this kit's ladder runs, so a renamed store never reads version 0 and
never replays its ladder against a v6 layout (the v5 → v6 rebuild folds every
row's generation to 0 and leaves a duplicate ledger row). `mootx01 upgrade`
is the vehicle.

Entries below this line predate the rename and name the kit as it was.
...
SynapseKit and required no callers to change.
...
is preserved in `the maintainer security audit (SynapseKit)`.
