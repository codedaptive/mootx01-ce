    ...
    func floatIndexSurvivesReopenSQLite() async throws {
      try await GlobalTestLock.shared.withLock {
        let url = FileManager.default.temporaryDirectory
            .appendingPathComponent("synapsekit-float-reopen-\(UUID().uuidString).sqlite3")
    ...
    func residencyParityIndexMatchesScan() async throws {
      try await GlobalTestLock.shared.withLock {
        ...
        let ramURL = FileManager.default.temporaryDirectory
            .appendingPathComponent("synapsekit-parity-ram-\(UUID().uuidString).sqlite3")
        let diskURL = FileManager.default.temporaryDirectory
            .appendingPathComponent("synapsekit-parity-disk-\(UUID().uuidString).sqlite3")
