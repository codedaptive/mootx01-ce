...
// (.uuid/.timestamp) on read, while SQLite — the backend production and the
// gauntlet actually use, and the on-disk equivalent of MemPalace's Chroma —
// hands back the primitive forms (.text/ISO-8601 text). Testing on the in-RAM
// backend hid real reopen bugs (vectors silently failing to decode). One scratch
// SQLite file per call, in the temp dir.
func makeScratchStorage() throws -> any Storage {
    let url = FileManager.default.temporaryDirectory
        .appendingPathComponent("synapsekit-test-\(UUID().uuidString).sqlite3")
