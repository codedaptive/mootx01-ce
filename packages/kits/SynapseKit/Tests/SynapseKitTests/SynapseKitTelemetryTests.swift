// SynapseKitTelemetryTests.swift
//
...
// packages/kits/SynapseKit/rust/tests/synapsekit_telemetry_tests.rs.
//
...
//    synapsekit.* namespace spec (VECTORKIT_REPORT_001).
...
//   enabled. Count assertions filter to the synapsekit.* namespace to
...
@Suite("§1 SynapseKitTelemetry — disabled gate", .serialized)
struct SynapseKitTelemetryDisabledTests {
...
@Suite("§2 SynapseKitTelemetry — enabled gate", .serialized)
struct SynapseKitTelemetryEnabledTests {
    ...
    func addVectorEmitsOneMetric() async throws {
        try await GlobalTestLock.shared.withLock {
            ...
            // Only assert on synapsekit.* emissions; lower-layer kits may
            // emit substrate.* metrics via the same enabled sink.
            #expect(sink.count(prefix: "synapsekit.") == 1,
                "addVector must emit exactly one synapsekit.* metric; got \(sink.count(prefix: "synapsekit."))")
    ...
    func findNearestEmitsTwoMetrics() async throws {
        try await GlobalTestLock.shared.withLock {
            ...
            // Count only synapsekit.* metrics. Lower-layer kits (e.g.
            // SubstrateKernel via EngramLib) may emit their own metrics
            // into this sink when monitoring is enabled.
            let vkCount = sink.count(prefix: "synapsekit.")
            #expect(vkCount == 2,
                "findNearest must emit 2 synapsekit.* metrics (latency + result_count); got \(vkCount)")
    ...
    func findByKeywordEmitsOneMetric() async throws {
        try await GlobalTestLock.shared.withLock {
            ...
            #expect(sink.count(prefix: "synapsekit.") == 1,
                "findByKeyword must emit exactly one synapsekit.* metric; got \(sink.count(prefix: "synapsekit."))")
...
/// value shapes. Verifies the synapsekit.* namespace contract
/// (VECTORKIT_REPORT_001).
@Suite("§3 SynapseKitTelemetry — metric shapes", .serialized)
struct SynapseKitTelemetryShapeTests {

    /// addVector emits synapsekit.index.insert_latency_ms with positive
    ...
    func addVectorLatencyMetricShape() async throws {
        try await GlobalTestLock.shared.withLock {
            ...
            let vkSamples = sink.samples(prefix: "synapsekit.")
            let hasShapedInsertMetric = vkSamples.contains { s in
                guard case let .metric(name, value, tags, _) = s else { return false }
                return name == "synapsekit.index.insert_latency_ms"
                    && value >= 0.0
                    && tags["kit"] == "SynapseKit"
                    && tags["model_id"] == "minilm"
            }
            #expect(hasShapedInsertMetric,
                "addVector must emit synapsekit.index.insert_latency_ms with value>=0, kit=SynapseKit, model_id=minilm")
    ...
    /// findNearest emits synapsekit.search.latency_ms and
    /// synapsekit.search.result_count with correct shapes.
    ...
    func findNearestMetricShapes() async throws {
        try await GlobalTestLock.shared.withLock {
            ...
            // Filter to synapsekit.* metrics only. Lower-layer kits (e.g.
            // SubstrateKernel via EngramLib) may emit their own metrics
            // into this sink; we assert only on the SynapseKit namespace.
            let vkSamples = sink.samples(prefix: "synapsekit.")
            #expect(vkSamples.count == 2,
                "findNearest must emit exactly 2 synapsekit.* metrics; got \(vkSamples.count)")

            // First SynapseKit metric: latency.
            if vkSamples.count >= 1,
               case let .metric(name, value, tags, _) = vkSamples[0] {
                #expect(name == "synapsekit.search.latency_ms",
                    "first synapsekit.* metric must be synapsekit.search.latency_ms")
                #expect(value >= 0.0, "latency_ms must be non-negative")
                #expect(tags["kit"] == "SynapseKit")
                #expect(tags["model_id"] == "minilm")
            }

            // Second SynapseKit metric: result count.
            if vkSamples.count >= 2,
               case let .metric(name, value, tags, _) = vkSamples[1] {
                #expect(name == "synapsekit.search.result_count",
                    "second synapsekit.* metric must be synapsekit.search.result_count")
    ...
    /// findByKeyword emits synapsekit.search.keyword_result_count with
    /// the correct count.
    ...
    func findByKeywordMetricShape() async throws {
        try await GlobalTestLock.shared.withLock {
            ...
            let hasShapedKeywordMetric = vkSamples.contains { s in
                guard case let .metric(name, value, tags, _) = s else { return false }
                return name == "synapsekit.search.keyword_result_count"
                    && value == 3.0
                    && tags["kit"] == "SynapseKit"
            }
            #expect(hasShapedKeywordMetric,
                "findByKeyword must emit synapsekit.search.keyword_result_count=3 with kit=SynapseKit")
...
@Suite("§4 SynapseKitTelemetry — conformance (results unchanged by telemetry)", .serialized)
struct SynapseKitTelemetryConformanceTests {
