...
//   or (b) calls a VectorStore method that emits (addVector, findNearest,
//   findByKeyword), must hold GlobalTestLock while monitoring is on.
//   Otherwise a concurrent test's VectorStore call will emit into the
...
//   }
//
// ALL tests in SynapseKitTests that call addVector, findNearest, or
// findByKeyword — or that toggle Intellectus.setEnabled — MUST use
// this lock. That includes both the telemetry test suite and the
// functional VectorStoreTests.
