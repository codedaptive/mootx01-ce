...
//! `EmbeddingProvider` in synapsekit. Rust mirror of the Swift
//! `SynapseKit.FloatSimHashEmbeddingProvider`.
//!
...
use crate::{EmbeddingProvider, SynapseKitError};
...
impl EmbeddingProvider for FloatSimHashEmbeddingProvider {
    ...
    fn embed(&self, text: &str) -> Result<Engram, SynapseKitError> {
        ...
        let floats = (self.inference)(text).map_err(SynapseKitError::EmbeddingFailed)?;
    ...
    fn embed_float(&self, text: &str) -> Result<Vec<f32>, SynapseKitError> {
        ...
        (self.inference)(text).map_err(SynapseKitError::EmbeddingFailed)
    }
}
