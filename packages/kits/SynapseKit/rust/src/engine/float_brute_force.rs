...
/// `SynapseKitError::InvalidPayload`.
///
...
impl DenseIndex for FloatBruteForceIndex {
    ...
    ///
    /// Returns `SynapseKitError::InvalidPayload` if `vector.kind` is not
...
mod tests {
    ...
    fn add_rejects_mixed_dimension_vector() {
        ...
        assert!(
            matches!(result.unwrap_err(), SynapseKitError::InvalidPayload(_)),
    ...
    fn scan_refuses_header_storage_mismatch() {
        ...
        assert!(
            matches!(result, Err(SynapseKitError::InvalidPayload(_))),
