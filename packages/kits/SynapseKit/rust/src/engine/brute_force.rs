...
//!
//! I-7 (arch spec §3.1 / §3.4): **SynapseKit performs NO Hamming math.**
...
/// metrics return `SynapseKitError::InvalidPayload`.
#[derive(Debug)]
...
impl BruteForceIndex {
    ...
    fn payload_to_engram(payload: &VectorPayload) -> Result<Engram, SynapseKitError> {
        ...
        Self::bytes_to_engram(&payload.bytes).ok_or_else(|| {
            SynapseKitError::InvalidPayload("BruteForceIndex: could not decode Engram".into())
        })
    }
...
impl DenseIndex for BruteForceIndex {
    ...
    fn build(
        ...
        for v in vectors {
            if v.kind != VectorKind::Binary || v.bytes.len() != 32 {
                return Err(SynapseKitError::InvalidPayload(
    ...
    fn remove(&mut self, key: &VectorRecordKey) -> Result<(), SynapseKitError> {
        let mut new_tombstones = self.array.tombstones.clone();
...
mod tests {
    ...
    fn non_binary_probe_returns_error() {
        ...
        assert!(matches!(res, Err(SynapseKitError::InvalidPayload(_))));
    }
