...
impl ResidentArrayStore {
    ...
    pub fn load(&mut self) -> Result<(), SynapseKitError> {
    ...
    pub fn flush(&mut self) -> Result<(), SynapseKitError> {
        if !self.is_dirty {
            return Ok(());
        }
    ...
    pub fn tombstone(&mut self, key: &VectorRecordKey) -> Result<(), SynapseKitError> {
        let mut new_tombstones = self.array.tombstones.clone();
    ...
    pub fn compact(&mut self) -> Result<(), SynapseKitError> {
    ...
    pub fn parse_sidecar(data: &[u8]) -> Result<ResidentVectorArray, SynapseKitError> {
        ...
        let kind = match kind_raw {
            ...
            2 => VectorKind::Int8,
            _ => return Err(SynapseKitError::DecodingFailure(
        ...
        let tombstone_block = tombstone_words.checked_mul(8).ok_or_else(|| {
            SynapseKitError::DecodingFailure(
...
fn decode_key(data: &[u8], pos: usize) -> Result<(VectorRecordKey, usize), SynapseKitError> {
    let mut p = pos;
...
fn decode_partitions(
    data: &[u8],
    pos: usize,
) -> Result<(Vec<ModelPartitionEntry>, usize), SynapseKitError> {
    if pos + 4 > data.len() {
...
fn check_bounds(
    ...
    if pos + len > data.len() {
        Err(SynapseKitError::DecodingFailure(format!(
...
fn read_le_u16(data: &[u8], pos: usize) -> Result<u16, SynapseKitError> {
    check_bounds(data, pos, 2, "u16")?;
    Ok(u16::from_le_bytes([data[pos], data[pos + 1]]))
}
fn read_le_u32(data: &[u8], pos: usize) -> Result<u32, SynapseKitError> {
