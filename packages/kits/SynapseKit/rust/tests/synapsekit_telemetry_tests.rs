//! SynapseKit telemetry integration tests — VECTORKIT_REPORT_001.
//!
//! Mirrors the Swift suite in
//! Tests/SynapseKitTests/SynapseKitTelemetryTests.swift.
...
//!   §3 Metric shapes: names, tags, and values match the synapsekit.* spec.
...
fn find_nearest_emits_two_metrics_when_enabled() {
    ...
    // Count only synapsekit.* metrics. Other kits lower in the dependency
    ...
    // on the SynapseKit emissions.
    let vk_count = sink.all_samples().into_iter().filter(|s| {
        if let StatSample::Metric { name, .. } = s { name.starts_with("synapsekit.") } else { false }
    }).count();
    assert_eq!(vk_count, 2,
        "find_nearest must emit 2 synapsekit.* metrics (latency + result_count); got {}", vk_count);
...
/// add_vector emits synapsekit.index.insert_latency_ms with correct shape.
#[test]
fn add_vector_insert_latency_metric_shape() {
    ...
    if let Some(StatSample::Metric { name, value, tags, .. }) = samples.first() {
        assert_eq!(name, "synapsekit.index.insert_latency_ms",
            "insert latency must be named synapsekit.index.insert_latency_ms");
        assert!(*value >= 0.0, "latency_ms must be non-negative; got {}", value);
        assert_eq!(tags.get("kit").map(|s| s.as_str()), Some("SynapseKit"),
            "insert latency must carry kit=SynapseKit tag");
...
/// find_nearest emits synapsekit.search.latency_ms and
...
fn find_nearest_metric_shapes() {
    ...
    if let Some(StatSample::Metric { name, value, tags, .. }) = vk_samples.get(1) {
        assert_eq!(name, "synapsekit.search.result_count");
...
/// find_by_keyword emits synapsekit.search.keyword_result_count with correct shape.
#[test]
fn find_by_keyword_metric_shape() {
    ...
    if let Some(StatSample::Metric { name, value, tags, .. }) = samples.first() {
        assert_eq!(name, "synapsekit.search.keyword_result_count");
        assert_eq!(*value, 3.0,
