...
//! `SynapseKitError::Int8QuantizationPolicyUndefined` on any int8 payload.
//!
...
use std::sync::Arc;
use synapsekit::{VectorKind, VectorPayload, SynapseKitError, VectorPayloadInput, VectorStore};
...
fn is_int8_policy_error(e: &SynapseKitError) -> bool {
    matches!(e, SynapseKitError::Int8QuantizationPolicyUndefined(_))
}
...
fn add_payload_int8_error_message_is_informative() {
    ...
    if let SynapseKitError::Int8QuantizationPolicyUndefined(msg) = err {
        assert!(
            msg.to_lowercase().contains("quantization"),
            "error message must mention quantization. Got: {msg}"
        );
        assert!(
            msg.contains("SYNAPSEKIT_SPEC"),
            "error message must cite SYNAPSEKIT_SPEC. Got: {msg}"
        );
    } else {
...
///
/// The symmetric read guard described in SYNAPSEKIT_SPEC §I-4a ensures that
