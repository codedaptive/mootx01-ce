...
struct ExcludedColumnsRoundTripTests {
    ...
    @Test("manifest with excludedColumns round-trips through JSON")
    func manifestRoundTrip() throws {
        ...
        #expect(decoded.table(named: "items")?.excludedColumns == ["score"])
        #expect(decoded.table(named: "log")?.excludedColumns.isEmpty == true)
    }
}
