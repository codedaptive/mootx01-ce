...
struct ColumnHLCMapConformanceTests {
    ...
    func conflictPolicyDecodeRustGolden() throws {
        let golden = """
        {"kitID":"TestKit","schemaVersion":1,"zoneIdentifier":"zone","tables":[{"name":"items","direction":"bidirectional","primaryKeyColumn":"id","conflictPolicy":"fieldLevelLWW"}]}
        """
        let manifest = try JSONDecoder().decode(SyncManifest.self, from: Data(golden.utf8))
        let table = try #require(manifest.tables.first)
        #expect(table.conflictPolicy == .fieldLevelLWW)
    }
}
