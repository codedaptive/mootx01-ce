...
//
// Final home: LocusKit. Re-homed from MootProGateway/Sync/ via MootDaemonFederation
// to LocusKit — the cross-platform (macOS + iOS) kit that owns AdjectiveSensitivity
// and adjectiveBitmap schema semantics. Both the daemon package (apps/mootx01) and the
// app package (apps/Mootx01-App) import LocusKit, so this is the only location from
// which both sides can reference a single implementation.
//
