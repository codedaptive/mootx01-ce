...
/// `distilled_at`) that are NULL together or populated together, written in
/// one atomic UPDATE, and cleared to NULL by every write that touches the
/// row's `content` (edit, dataset patch, expunge scrub). No Bool columns;
/// no entity-level "isDistilled" accessor — callers test `distilled != nil`.
///
