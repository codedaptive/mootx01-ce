//! Cross-port conformance for the trainable-basis seam — RI cases (always on).
//!
...
//! 6a-i committed Swift canonical basis blob BYTE-FOR-BYTE for RandomIndexing.
...
//!
//! LSA seam tests live in `lsa_seam_tests.rs` (gated by the `lsa` feature).
//!
//! Also exercises `EmbeddingModelConfig::reconstruct` for RI: the trainable
//! case round-trips a basis blob to embeddings identical to the trained
//! provider's, and the non-trainable case (Deterministic) returns
...
use corpus_kit_providers::{RandomIndexingProvider, RI_PROJECTION_SEED};
...
/// RI fixture stores the training corpus as token arrays.
#[derive(Deserialize)]
...
//
// Drives the counts seam THROUGH the trait: `add_to_counts` per chunk grows
// the maintained vocabulary anchor, and `serialize_counts` → `restore_counts`
// resumes that anchor in a fresh provider. The Swift twin
