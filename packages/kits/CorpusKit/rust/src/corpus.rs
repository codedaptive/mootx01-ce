...
impl Corpus {
    ...
    /// Implicit retrain triggers in `ingest` are: (a) first-ingest when no basis
    /// exists, and (b) growth-retrain when the corpus has doubled since the last
    /// train and the basis is still young (Kinsta-fix). Explicit `reindex` trains
    /// whatever trainable handle is present at open.)
    ...
    ///   (b) growth retrain (Kinsta-fix): basis is young
    ///       (trained_chunk_count < PER_DOC_AUTO_RETRAIN_STABLE_CHUNK_THRESHOLD)
    ///       AND corpus has grown to ≥ 2× trained_chunk_count → retrain on the
    ///       full corpus. Stops once the basis is stable. Prevents a rank-1 LSA
    ///       SVD from persisting on a 1-doc first-ingest corpus.
    ///
