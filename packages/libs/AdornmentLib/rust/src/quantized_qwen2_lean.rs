//! Lean vendored copy of candle-transformers 0.9.2 `quantized_qwen2`
//! (Apache-2.0/MIT) for the gold miner's residency budget
//! (ADORNMENTLIB_SPEC 0.5.0 § Gold miner).
//!
//! ONE functional change from upstream: the token-embedding table is held
//! at F16 instead of F32. Upstream dequantizes the full table to F32 —
//! on Qwen2's 151k vocabulary that alone is ~545 MB, which blew the
//! miner's enforced sub-gigabyte budget. F16 halves it; the embedding
//! lookup result is cast back to F32 before the first layer so every
//! other tensor path is byte-identical to upstream. Delete this file and
//! re-import upstream if candle ever grows a lean-embedding option.
//!
...
impl ModelWeights {
    pub fn from_gguf<R: std::io::Seek + std::io::Read>(
        ...
        // LEAN CHANGE: dequantize the table DIRECTLY to F16 (the upstream
        // F32 table is ~545 MB at Qwen2's vocab size; F16 halves it and
        // dequantize_f16 avoids even a transient F32 copy).
    ...
    pub fn forward(&mut self, x: &Tensor, index_pos: usize) -> Result<Tensor> {
        ...
        // LEAN CHANGE: lookup happens on the F16 table; cast the looked-up
        // rows (tiny — seq_len × hidden) back to F32 so the layer stack is
