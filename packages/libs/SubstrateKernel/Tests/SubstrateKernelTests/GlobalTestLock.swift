...
//
// Process-wide serialisation lock for SubstrateKernelTests that touch the
...
//   or (b) calls PortableKernel.kernelForCurrentPlatform() — the package's
//   one emit site (backend_selected) — must hold GlobalTestLock while it
//   runs. Otherwise a concurrent test's factory call emits into the
//   capturing sink and corrupts exact-count assertions (observed in the
//   test-full lane: "conformance holds when monitoring is enabled" saw
//   sink.count == 2).
//
...
//   }
//
// ALL tests in SubstrateKernelTests that toggle Intellectus.setEnabled or
// call kernelForCurrentPlatform() MUST use this lock.
