...
final class AuditGateTests: XCTestCase {
    ...
    func testNoClobber() {
        ...
        guard case .success(let ev) = r else { return XCTFail("expected admit") }
        XCTAssertEqual((ev.afterBitmaps.adjective >> 18) & 0x3F, 3)   // trust preserved
        XCTAssertEqual(ev.afterBitmaps.operational & 0xF, 5)          // channel written
        XCTAssertEqual((ev.afterBitmaps.operational >> 4) & 0xF, 0xF) // neighbor bits preserved
        XCTAssertEqual(ev.verb, "mutate")
    }

    func testUndeclaredFieldRejected() {
        ...
        let r = AuditGate.admit(estateUuid: estate, rowId: row, nounType: .drawer, verb: .mutate,
            ...
            afterLatticeAnchor: anchor, vocabulary: vocab([channel]), hlc: hlc(1), actor: "t")
        guard case .failure(.undeclaredField(let l)) = r else { return XCTFail("expected undeclaredField") }
        XCTAssertEqual(l, "undeclared")
    }

    func testIllegalValueRejected() {
        ...
        let r = AuditGate.admit(estateUuid: estate, rowId: row, nounType: .drawer, verb: .mutate,
            ...
            afterLatticeAnchor: anchor, vocabulary: vocab([channel]), hlc: hlc(1), actor: "t")
        guard case .failure(.illegalValue(_, let v)) = r else { return XCTFail("expected illegalValue") }
        XCTAssertEqual(v, 7)
    }

    func testOverWidthValueRejectedNotTruncated() {
        ...
        guard case .failure(.illegalValue(_, let v)) = r else { return XCTFail("expected illegalValue (no truncation)") }
        XCTAssertEqual(v, 20)
    }
    ...
    func testIllegalTransitionRejected() {
        ...
        guard case .failure(.basisViolation) = r else { return XCTFail("expected basisViolation") }
    }

    func testContentIDDeterministicAndStable() {
        ...
        guard case .success(let a) = mk(), case .success(let b) = mk() else { return XCTFail() }
        XCTAssertEqual(a.eventID, b.eventID)  // same logical event ⇒ same ID (idempotence)
    }

    func testContentIDChangesWithPayload() {
        let mk = { (v: Int64) in AuditGate.admit(estateUuid: self.estate, rowId: self.row, nounType: .drawer, verb: .capture,
            ...
            afterLatticeAnchor: self.anchor, vocabulary: self.vocab([self.channel]), hlc: self.hlc(7), actor: "t") }
        guard case .success(let a) = mk(2), case .success(let b) = mk(3) else { return XCTFail() }
        XCTAssertNotEqual(a.eventID, b.eventID)
    }

    // MARK: vocabulary freeze

    func testFreezeRejectsOverlap() {
        ...
        guard case .failure(.overlap) = VocabularyValidator.freeze(union: [a, b]) else {
            return XCTFail("expected overlap")
        }
    }

    func testFreezeRejectsBasisCollision() {
        ...
        guard case .failure(.basisCollision) = VocabularyValidator.freeze(union: [collide]) else {
            return XCTFail("expected basisCollision")
        }
    }

    func testFreezeRejectsMalformedWidth() {
        ...
        guard case .failure(.malformedWidth) = VocabularyValidator.freeze(union: [bad]) else {
            return XCTFail("expected malformedWidth")
        }
    }

    func testFreezeRejectsValueExceedingWidth() {
        ...
        guard case .failure(.valueExceedsWidth) = VocabularyValidator.freeze(union: [bad]) else {
            return XCTFail("expected valueExceedsWidth")
        }
    }

    func testFreezeAcceptsCleanUnion() {
        guard case .success = VocabularyValidator.freeze(union: [channel]) else {
            return XCTFail("clean union should freeze")
        }
    }
    ...
    func testContentIDSharedVector() {
        ...
        XCTAssertEqual(hex, "eba1f4509f84abe2a472d99fb621334b")
    }
    // State cannot be moved to a state the verb did not authorize.
    func testStateInconsistentWithVerbRejected() {
        ...
        guard case .failure(.stateInconsistentWithVerb) = r else {
            return XCTFail("expected stateInconsistentWithVerb")
        }
    }

    // A basis value field refuses a non-enumerated value (sensitivity=7).
    func testBasisValueRejected() {
        ...
        let r = AuditGate.admit(estateUuid: estate, rowId: row, nounType: .drawer, verb: .mutate,
            ...
            afterLatticeAnchor: anchor, vocabulary: vocab(), hlc: hlc(1), actor: "t")
        guard case .failure(.illegalValue(let l, 7)) = r else { return XCTFail("expected illegalValue") }
        XCTAssertEqual(l, "sensitivity")
    }

    // Capture must use the capture verb; a mutation verb on a fresh row is refused.
    func testCaptureRequiresCaptureVerb() {
        let r = AuditGate.admit(estateUuid: estate, rowId: row, nounType: .drawer, verb: .mutate,
            prior: nil, priorLatticeAnchor: nil, writes: [FieldWrite(slot: channel, value: 1)],
            afterLatticeAnchor: anchor, vocabulary: vocab([channel]), hlc: hlc(1), actor: "t")
        guard case .failure(.stateInconsistentWithVerb) = r else {
            return XCTFail("expected stateInconsistentWithVerb for non-capture verb on fresh row")
        }
    }
    ...
    func testI22SecretCannotBeExportableViaGate() {
        ...
        guard case .failure(.basisViolation) = r else {
            return XCTFail("expected I-22 basisViolation for secret+exportable")
        }
    }
}
