...
// one file serves both readers: the kit resolves it package-locally, and the
// benchmarker reaches the same path by walking up to the repository root.
...
/// this test file's location. NeuronKit owns the fixture so that the kit and
/// the benchmarker assert against one file. The benchmarker lives at
/// benchmarks/ and walks up to the repository root to reach it:
///   .../benchmarks/Tests/mcp-benchmarkerTests/AblationColumnsTests.swift
...
///
/// The deletion count below follows this package's flat layout; each step is
/// named in the ladder above.
private func compositionGridFixturePath(file: String = #filePath) -> URL {
    URL(fileURLWithPath: file)
        .deletingLastPathComponent()   // mcp-benchmarkerTests/
        .deletingLastPathComponent()   // Tests/
        .deletingLastPathComponent()   // benchmarks/ (the package root sits at repo root)
...
struct AblationColumnsTests {
    ...
    @Test("columns include the three search strategies and every composition")
    func columnSet() throws {
        let expected = try fixtureCompositions
        // Core columns carry no baseline column; a two-endpoint extension
        // lane injects one via columns(baselineName:).
        let cols = GauntletRunner.columns(baselineName: nil)
        ...
        // Exactly one column per composition; no duplicates, no baseline.
        #expect(cols.count == MootScoring.allCases.count + expected.count)
        #expect(cols.allSatisfy { $0.isMootx01 })
    }
    ...
    func provenanceInHeader() {
        ...
        report.columnsRun = ["mootx01:raw", "precise:text"]
        ...
        #expect(rendered.contains("mootx01:raw"))
