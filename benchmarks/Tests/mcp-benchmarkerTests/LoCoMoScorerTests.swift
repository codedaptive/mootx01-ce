...
final class LoCoMoScorerTests: XCTestCase {
    ...
    private func loadLoCoMoVectors() throws -> [String: Any] {
        ...
        // #filePath → .../benchmarks/Tests/mcp-benchmarkerTests/LoCoMoScorerTests.swift
        // parent → .../Tests/mcp-benchmarkerTests/
        // parent → .../Tests/
        // parent → .../benchmarks/ (package root)
    ...
    func testBuildLoCoMoReportPopulatesProvenanceFields() throws {
        ...
        let report = buildLoCoMoReport(config: config, corpus: corpus, results: [], scores: [score], rerankFailures: 0)
        ...
        // 4. payloadText is persisted verbatim in the per-question row so that
        //    retrieval diagnosis can read which chunks the recall verb actually
        //    returned without requiring a re-run with instrumentation.
        XCTAssertEqual(q.payloadText, payload,
            "payloadText in the report per-question row must equal the raw payload " +
            "that was captured by the runner — the field must not be dropped anywhere " +
            "in the runner→scorer→report chain")
    }

    /// Pinning test: payload text survives runner → scorer → report per-question row.
    ///
    /// Exercises the full scoreLoCoMoQuestion + buildLoCoMoReport chain with a
    /// non-nil payloadText so that any future refactor that accidentally drops the
    /// field will be caught immediately rather than requiring a live benchmark re-run
    /// with extra instrumentation to diagnose the silence.
    func testPayloadTextSurvivesToPerQuestionRow() throws {
        let rawPayload = "found 2 memory(s)\nfirst chunk text\nsecond chunk text"

        // Construct a minimal result that carries payloadText.
        let result = LoCoMoQuestionResult(
            questionID: "payload-pin-q1",
            conversationIndex: 0,
            categoryLabel: "single_hop",
            category: 1,
            queryLatencySeconds: 0.08,
            retrievedUUIDs: ["uuid-A", "uuid-B"],
            manifest: [
                LoCoMoManifestEntry(uuid: "uuid-A", diaID: "D1:1",
                                    sessionNumber: 1, turnIndex: 0, speaker: "Alice"),
                LoCoMoManifestEntry(uuid: "uuid-B", diaID: "D1:2",
                                    sessionNumber: 1, turnIndex: 1, speaker: "Bob"),
            ],
            evidenceDiaIDs: ["D1:1"],
            guardHealthy: true,
            guardDiagnostic: nil,
        ...
        // Thread result through the scorer.
        let score = scoreLoCoMoQuestion(result)
        XCTAssertEqual(score.payloadText, rawPayload,
            "scoreLoCoMoQuestion must pass payloadText through unchanged")

        // Thread score through the report builder.
        let config = LoCoMoRunConfig(
            mootBinaryPath: "/unused",
            datasetPath: URL(fileURLWithPath: "/unused"),
            limit: nil, offset: 0, seed: 42, outDir: nil,
            runLabel: "payload-pin-test",
            encodeBarrier: .drain,
            estateCache: .off, cacheDir: nil,
            scratchPosture: .plaintextTransient,
            strategy: .search,
            categoryFilter: nil, recallShape: nil, rerankCmd: nil,
            seedPath: .batch
        )
        let corpus = LoCoMoCorpus(conversations: [], questions: [], adversarialCount: 0)
        let report = buildLoCoMoReport(
            config: config, corpus: corpus,
            results: [result], scores: [score],
            rerankFailures: 0
        )

        guard let row = report.perQuestion.first else {
            XCTFail("perQuestion array must not be empty"); return
        }

        // The field must arrive verbatim — not nil, not transformed.
        XCTAssertEqual(row.payloadText, rawPayload,
            "payloadText in LoCoMoReportPerQuestion must equal the original MCP payload " +
