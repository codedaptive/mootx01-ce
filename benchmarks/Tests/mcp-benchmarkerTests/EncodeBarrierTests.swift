...
final class DrainBarrierStateTests: XCTestCase {
    ...
    // MARK: - Non-gating lanes (probe-1 stall regression)

    /// The exact shape probe 1 hung on: corpus_encode idle, distillation
    /// pinned at a non-zero eligibility count with in_flight 0. The
    /// distillation lane is not queue work, so the barrier must report idle
    /// instead of polling until timeout.
    func test_distillationPendingDoesNotHoldBarrierOpen() {
    ...
    /// A non-gating lane never masks real queue work on a gating lane.
    func test_distillationSkippedButCorpusEncodeStillGates() {
        let text = "drains: 2\n  corpus_encode: draining \u{2014} pending: 12, in_flight: 2"
            + "\n  distillation: draining \u{2014} pending: 7, in_flight: 0, pipeline: p1"
        XCTAssertEqual(parseDrainResponse(text), .draining)
    }

    /// An UNKNOWN lane still gates — the denylist is deliberately narrow, so
    /// a lane we have not proven to be non-queue still holds the barrier.
    func test_unknownLaneStillGates() {
        let text = "drains: 2\n  corpus_encode: idle \u{2014} pending: 0, in_flight: 0"
            + "\n  lsa_encode: draining \u{2014} pending: 12, in_flight: 2"
        XCTAssertEqual(parseDrainResponse(text), .draining)
    }

    /// A malformed non-gating line is still a protocol error: skipping the
    /// lane for gating purposes must not skip validating it.
    func test_malformedNonGatingLineIsUnparseable() {
        let text = "drains: 2\n  corpus_encode: idle \u{2014} pending: 0, in_flight: 0"
    ...
    /// A crashed server does not announce itself. The transport reports
    /// "response timeout for request id N" and every later poll reports the
    /// same, so the wording list (session closed / not connected /
    /// disconnected / broken pipe) matches nothing and the barrier polls for
    /// its full 300 s timeout — twice over, in the run that found this.
    ///
    /// `consecutiveErrors` is the wording-INDEPENDENT signal: a live server
    /// answers moot_drain_status in milliseconds even while draining, so a
    /// run of failures is silence, not load.
    func test_consecutiveErrors_countsAcrossFailedPolls() {
        var state = DrainBarrierState(start: Date(), grace: grace)
        XCTAssertEqual(state.consecutiveErrors, 0)
        for expected in 1...12 {
            state.noteError()
            XCTAssertEqual(state.consecutiveErrors, expected)
        }
    }

    /// One good answer ends the streak: a transient failure must not
    /// accumulate toward the death threshold across an otherwise healthy run.
    func test_responseClearsTheErrorStreak() {
        let start = Date()
        var state = DrainBarrierState(start: start, grace: grace)
        state.noteError()
        state.noteError()
        XCTAssertEqual(state.consecutiveErrors, 2)
        state.noteResponded()
        XCTAssertEqual(state.consecutiveErrors, 0,
...
// MARK: - dreamStatusFromDrainText (item 8)

final class DreamStatusTests: XCTestCase {

    func test_drainsNone_returnsDefault() {
        let status = dreamStatusFromDrainText("drains: none")
        XCTAssertEqual(status.pending, 0)
        XCTAssertTrue(status.settled)
    }

    func test_absentLane_returnsDefault() {
        let text = "drains: 1\n  corpus_encode: idle \u{2014} pending: 0, in_flight: 0"
        let status = dreamStatusFromDrainText(text)
        XCTAssertEqual(status.pending, 0)
        XCTAssertTrue(status.settled)
    }

    func test_drainingLane_parsesFields() {
        let text = "drains: 2\n  corpus_encode: idle \u{2014} pending: 0, in_flight: 0\n  dreaming: draining \u{2014} pending: 17, in_flight: 2"
        let status = dreamStatusFromDrainText(text)
        XCTAssertEqual(status.pending, 17)
        XCTAssertFalse(status.settled)
    }

    func test_idleLane_settled() {
