...
/// Resolves `benchmarks/conformance/<filename>` from this test file:
...
struct LMEScorerInfrastructureTests {
    ...
    func reportRoundTrip() throws {
        ...
        let report = LMEReport(
            ...
            synthesizeCell: LMESynthesizeCell(
                enabled: false, questionCount: 0, judgedCount: nil, accuracyRate: nil),
            // MXE-BK. Every value here is deliberately NOT the default so a
            // dropped field or a misspelled CodingKey surfaces as a failed
            // assertion rather than a value that happens to match the zero case.
            runParameters: LMEReportRunParameters(
                judgeHydrationDepth: 30,
                judgeGrading: "verdict",
                judgeCmdSet: true,
                arm: "both",
                exactStrategy: "auto",
                recallShape: "balanced",
                ...
                synthesizeLimit: 200,
                slice: "dev",
                // W2-rerank: set true (not the zero/false default) so a dropped
                // CodingKey surfaces as a failed assertion.
                rerankCmdSet: true),
            // W2-rerank: 2 (not nil/0) so a dropped field surfaces as a failure.
            rerankFailures: 2,
            identityEnvironment: nil,
            timingReport: nil,
            timingSampling: "once-per-leg",
            // Topology labels are lane constants; the round trip checks the
            // CodingKeys spellings, not value variety.
            estateShape: "standard",
        ...
        // Asserted on the raw JSON as well as the decoded struct because the
        // failure mode being guarded is a missing or misspelled CodingKey,
        // which a struct-only round-trip would not catch. The value is the
        // lane constant false (the extraction mode is retired).
        #expect(rawJSON["preference_extraction"] as? Bool == false,
                "additive key 'preference_extraction' must be present in the report JSON")
        #expect(decoded.preferenceExtraction == false,
        ...
        // MXE-BK additive block: every option that moves the accuracy number.
        // Asserted on the raw JSON as well as the decoded struct because the
        // failure mode being guarded is a missing or misspelled CodingKey,
        // which a struct-only round-trip would not catch.
        let rpJSON = try #require(rawJSON["run_parameters"] as? [String: Any],
                                  "additive key 'run_parameters' must be present")
        #expect(rpJSON["judge_hydration_depth"] as? Int == 30,
                "judge_hydration_depth is the parameter this block was added for")
        #expect(rpJSON["judge_grading"] as? String == "verdict")
        #expect(rpJSON["arm"] as? String == "both")
        #expect(rpJSON["exact_strategy"] as? String == "auto")
        #expect(rpJSON["recall_shape"] as? String == "balanced")
        #expect(rpJSON["fresh_per_question"] as? Bool == true)
        #expect(rpJSON["seed"] as? UInt64 == 20_260_725)
        #expect(rpJSON["limit"] as? Int == 10)
        #expect(rpJSON["offset"] as? Int == 5)
        #expect(rpJSON["synthesize_limit"] as? Int == 200,
                "an explicit --synthesize-limit must reach the report; the synthesize ranking trials were distinguishable only by this flag")
        #expect(decoded.runParameters.judgeHydrationDepth == 30,
                "judge_hydration_depth must survive the encode/decode round trip")
        #expect(decoded.runParameters.judgeGrading == "verdict")
        // The judge COMMAND is recorded as presence ONLY (MXE-JD). Its text is
        // user-supplied shell that routinely carries API keys, so neither it
        // nor anything derived from it may reach a published report — any
        // fingerprint of the command, however hashed, lets a report recipient
        // confirm offline guesses of it. Pinned so nobody adds a "safer"
        // digest or "completes" the set with the raw string.
        #expect(rpJSON["judge_cmd_set"] as? Bool == true)
        #expect(rpJSON["judge_cmd_digest"] == nil,
                "no fingerprint of the judge command may appear in the report")
        #expect(rpJSON["judge_cmd"] == nil,
                "the judge command string must NEVER be persisted to the report")
        // W2-rerank: rerank_cmd_set must be PRESENCE ONLY (same secrecy law as judge_cmd).
        #expect(rpJSON["rerank_cmd_set"] as? Bool == true,
                "rerank_cmd_set must be present in run_parameters (W2-rerank)")
        #expect(rpJSON["rerank_cmd"] == nil,
                "the rerank command string must NEVER be persisted to the report")
        #expect(rawJSON["rerank_failures"] as? Int == 2,
                "rerank_failures must appear at the report top level when rerankCmd was active")
        ...
        // C10: Shape 3 deviation label keys. Asserted on the raw JSON as well as the
        // decoded struct because the failure mode is a missing or misspelled CodingKey.
        #expect(rawJSON["estate_shape"] as? String == "standard",
                "additive key 'estate_shape' must be present in the report JSON")
        #expect(rawJSON["protocol_deviation"] as? Bool == false,
                "additive key 'protocol_deviation' must be present in the report JSON")
        // Decoded struct round-trip.
        #expect(decoded.estateShape == "standard",
                "estate_shape must survive the encode/decode round trip")
        #expect(decoded.protocolDeviation == false,
                "protocol_deviation must survive the encode/decode round trip")
    }

    // MARK: Run parameters — judge command leaves no trace (MXE-JD)

    /// Builds the run-parameters block from a judge command value exactly the
    /// way `buildLMEReport` does: presence only, nothing derived. Every other
    /// value is fixed so the only input that varies between calls is the
    /// command itself.
    private func makeRunParameters(judgeCmd: String?) -> LMEReportRunParameters {
    ...
    @Test("a run without a judge command records judge_cmd_set false and no digest")
    func runParametersRecordAbsentJudgeCommandAsFalse() throws {
        let data = try JSONEncoder().encode(makeRunParameters(judgeCmd: nil))
        let json = try #require(
            try JSONSerialization.jsonObject(with: data) as? [String: Any])
        #expect(json["judge_cmd_set"] as? Bool == false)
        #expect(json["judge_cmd_digest"] == nil,
                "no fingerprint of the judge command may appear in the report")
    }

    @Test("two runs with different judge commands produce identical run_parameters")
    func runParametersDoNotDistinguishJudgeCommands() throws {
        // This indistinguishability is the point of MXE-JD: the report can no
        // longer tell two judge commands apart, and that is the intended
        // loss. Any field that COULD distinguish them would necessarily be
        // derived from a secret-bearing string, which is exactly what lets a
        // report recipient confirm guesses of the command offline.
        let encoder = JSONEncoder()
        encoder.outputFormatting = [.sortedKeys]
        let a = try encoder.encode(
            makeRunParameters(judgeCmd: "claude -p --api-key guessable-low-entropy"))
        let b = try encoder.encode(
            makeRunParameters(judgeCmd: "ollama run llama3 -"))
        #expect(a == b, "run_parameters must not distinguish judge commands")
    }

    // MARK: synthesize_limit arm-gating (A2)

    /// Verifies the doc comment on `synthesizeLimit` in `LMEReportRunParameters`:
    /// "Nil = flag absent (tool default applies) or arm off."
    ///
    /// When `synthesizeArm` is false, `buildLMEReport` must write nil regardless
    /// of what `config.synthesizeLimit` holds. Without this gate, two runs that
    /// differed only in whether the arm was on produced `synthesize_limit` values
    /// that were identical (the flag value) even though the arm never fired.
    @Test("buildLMEReport omits synthesize_limit when synthesize arm is off, even with flag set")
    func synthesizeLimitOmittedWhenArmOff() throws {
        // Config: arm off, limit flag explicitly set to 200. The arm-off gate must
        // zero out the limit so the published report cannot suggest the arm ran.
        let config = LMERunConfig(
            ...
            rerankCmd: nil,
            synthesizeArm: false,
            synthesizeLimit: 200,   // flag IS set, but arm is off
            dumpJudgeInputsPath: nil,
            slice: nil
        )
        let corpus = LMECorpus(questions: [], abstentionCount: 0)
        let report = buildLMEReport(
            config: config, corpus: corpus,
            results: [], scores: [], rerankFailures: 0
        )
        ...
        let json = try #require(
            try JSONSerialization.jsonObject(with: data) as? [String: Any])
        let rpJSON = try #require(
            json["run_parameters"] as? [String: Any],
            "run_parameters block must be present")

        #expect(rpJSON["synthesize_limit"] == nil,
                "synthesize_limit must be absent when synthesize arm is off, even when --synthesize-limit flag was set")
    }
    ...
    func scoreGuardExcluded() {
        let result = LMEQuestionResult(
            ...
            cacheHit: nil,
            drainLaneObserved: nil,
            synthesizePayloadText: nil,
            synthesizeJudgeAnswer: nil,
            synthesizeJudgeCorrect: nil,
            synthesizeJudgeTokens: nil,
...
/// `buildLMEReport` labels every run with the lane's constant topology:
/// estate_shape="standard", protocol_deviation=false. The keys are report
/// contract; the values are constants of the measure-only lane.
@Suite("LME estate topology label")
struct LMEEstateTopologyLabelTests {

    /// Builds a minimal LMERunConfig for report-label testing.
    private func makeConfig() -> LMERunConfig {
        LMERunConfig(
            mootBinaryPath: "/dev/null",
            datasetPath: URL(fileURLWithPath: "/dev/null"),
            variant: "s",
    ...
    @Test("estate_shape=standard, protocol_deviation=false on every run")
    func topologyLabelsStandard() throws {
        ...
        #expect(report.estateShape == "standard",
                "the lane's only topology is one restored artifact per question")
        #expect(report.protocolDeviation == false,
                "restored-artifact-per-question is the published per-item protocol")

        // Verify the JSON keys are present with correct values (report contract).
        let encoder = JSONEncoder()
        encoder.outputFormatting = [.sortedKeys]
        let data = try encoder.encode(report)
        let raw = try #require(try JSONSerialization.jsonObject(with: data) as? [String: Any])
        #expect(raw["estate_shape"] as? String == "standard",
                "estate_shape key must be present in the report JSON")
        #expect(raw["protocol_deviation"] as? Bool == false,
                "protocol_deviation key must be present in the report JSON")
    }
}
