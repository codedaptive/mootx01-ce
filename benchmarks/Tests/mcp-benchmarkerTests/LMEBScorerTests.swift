...
// MARK: - UUID → doc ID mapping (lmebRankedDocsAudited conformance)

@Suite("LMEB scorer: lmebRankedDocsAudited conformance vectors")
struct LMEBRankedDocsConformanceTests {

    /// Verifies that unmapped UUIDs keep their rank slot as NUL-prefixed
    /// placeholders, matching the vectors in conformance/lmeb_vectors.json.
    ///
    /// This test pins the fix for the defect where lmebRankedDocs (inline in
    /// the runner) dropped unmapped UUIDs and collapsed rank gaps, causing
    /// every metric (nDCG, MRR, recall@k, AP) to overstate quality.
    @Test("All ranked_doc_cases: unmapped UUID holds its rank slot")
    func allRankedDocVectors() throws {
        let json = try loadLMEBConformanceJSON()
        let cases = try #require(json["ranked_doc_cases"] as? [[String: Any]])

        for c in cases {
            let id = c["id"] as? String ?? "(unknown)"
            let uuids = (c["retrieved_uuids"] as? [String]) ?? []
            let rawManifest = (c["manifest"] as? [[String: String]]) ?? []
            let manifest = rawManifest.map {
                LMEBManifestEntry(uuid: $0["uuid"]!, docID: $0["doc_id"]!)
            }
            let expected = (c["expected_ranked_doc_ids"] as? [String]) ?? []

            let (got, _) = lmebRankedDocsAudited(uuids: uuids, manifest: manifest)
            #expect(got == expected,
                    "ranked_doc_cases vector '\(id)': expected \(expected), got \(got)")
        }
    }
}
