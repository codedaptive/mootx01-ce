...
//! IO. The mock speaks a `search` shape (`results` array under a text block)
//! so the parse path is exercised too.
//!
...
/// JSON-RPC `result` envelopes; the `tools/call` reply carries an external
/// server `search`-shaped payload in a text content block.
...
while IFS= read -r line; do
  if [ "$first" = "1" ]; then
    ...
    # tools/call reply: external server search shape (results array in a text block)
...
fn stdio_client_connects_and_calls_tool() {
    ...
    // The external server search shape parses to two content items, no ids.
