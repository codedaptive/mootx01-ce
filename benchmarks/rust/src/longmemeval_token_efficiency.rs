...
//! of a hit (a corpus without `has_answer` annotations yields no evidence).
...
/// Returns true when the normalized gold answer appears in the normalized
/// judge payload — the retrieval ceiling for the judged metric. Reading this
/// next to the accuracy separates "moot did not retrieve it" from "the judge
/// could not read it". Upper bound only — short gold answers over-report.
///
/// Twin of Swift `lmeGoldAnswerInPayload(goldAnswer:payloadText:)`.
pub fn lme_gold_answer_in_payload(gold_answer: &str, payload_text: &str) -> bool {
    let normalized_gold = lme_normalize_for_evidence(gold_answer);
    if normalized_gold.is_empty() {
        return false;
    }
    lme_normalize_for_evidence(payload_text).contains(normalized_gold.as_str())
}
...
mod tests {
    ...
    /// reaches `benchmarks/`, where `conformance/` lives.
