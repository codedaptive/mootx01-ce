...
pub fn load_lmeb_corpus(
    ...
    // six. Keying these maps by the raw id made the last category loaded
    // silently overwrite the earlier ones — a corpus that looked complete and
    // had lost five sixths of its items, with no error anywhere. The same
    // collision reached the artifact store, where units are addressed by query
    // id.
    //
    // Separator is "__", not "/": these ids become seed FILENAMES and artifact
    // directory names. Twin of Swift `namespaced` in LMEBCorpus.swift.
    fn namespaced(et: &str, id: &str) -> String {
        format!("{et}__{id}")
    }
    ...
    for et in evidence_types {
        ...
        for (i, raw) in raw_docs.into_iter().enumerate() {
            ...
            let doc_id = namespaced(label, &raw.id);
            docs_by_id.insert(
                doc_id.clone(),
                LmebDoc { id: doc_id, text: raw.text, title: raw.title },
            );
        }
        ...
        for (i, raw) in raw_queries.into_iter().enumerate() {
            ...
            let query_id = namespaced(label, &raw.id);
            queries_by_id
                .insert(query_id.clone(), LmebQuery { id: query_id, text: raw.text });
        }
        ...
        for (i, raw) in raw_cands.into_iter().enumerate() {
            ...
            candidates_by_scene_id.insert(
                namespaced(label, &raw.scene_id),
                raw.candidate_doc_ids
                    .iter()
                    .map(|d| namespaced(label, d))
                    .collect(),
            );
        }
        ...
        for (query_id, doc_ids) in qrels {
            relevant_docs_by_query_id
                .entry(namespaced(label, &query_id))
                .or_default()
                .extend(doc_ids.iter().map(|d| namespaced(label, d)));
        }
    }
...
///
/// Twin of Swift `lmebCorpusDigest(baseDir:evidenceTypes:)` in `LMEBCorpus.swift`.
/// The byte layout of `combined` (sort order, filename list, key format `et/name=digest;`,
/// separator) is a cross-port contract: any change here must be mirrored in the Swift
/// twin, and vice versa.
///
/// Algorithm:
/// 1. Sort evidence types in ASCII ascending byte order. Evidence-type identifiers
///    are ASCII lowercase plus underscore, so Rust `sort_unstable` and Swift `sorted()`
///    agree on every valid input — this agreement is load-bearing across ports.
...
/// 3. Return `sha256_hex` of the UTF-8 bytes of the concatenated `combined` string.
///
/// The `"unknown"` failure mode is the point of this function: `"unknown"` never
/// validates in `ArtifactProvenance::mismatches` (`artifact_manifest.rs:116-117`),
...
pub fn lmeb_corpus_digest(base_dir: &Path, evidence_types: &[&str]) -> String {
    let mut sorted = evidence_types.to_vec();
    // ASCII byte-order sort: evidence-type identifiers are lowercase + underscore,
    // so this matches Swift's String.sorted() on the same inputs — cross-port contract.
    ...
    for et in &sorted {
        ...
        // contract: changing it here requires the same change in the Swift twin in
        // LMEBCorpus.swift. Both ports concatenate identically because both iterate
        // this fixed list in this fixed order.
        for name in ["candidates.jsonl", "corpus.jsonl", "qrels.tsv", "queries.jsonl"] {
            let path = base_dir.join(et).join(name);
            match crate::run_environment::file_sha256_hex(&path.to_string_lossy()) {
                Some(d) => combined.push_str(&format!("{et}/{name}={d};")),
                // Any unreadable required file makes the whole digest "unknown".
                // "unknown" never validates in ArtifactProvenance::mismatches
                // (artifact_manifest.rs:116-117), which is the intended behaviour:
                // an incomplete corpus must not silently produce a stable-looking digest.
                None => return "unknown".to_string(),
            }
        }
    }
    crate::run_environment::sha256_hex(combined.as_bytes())
}
...
mod tests {
    ...
    fn doc_contents_correct() {
        ...
        let doc = corpus.docs_by_id.get("user_evidence__scene_0_session_1_turn_1").unwrap();
        assert_eq!(doc.id, "user_evidence__scene_0_session_1_turn_1");
    ...
    fn candidate_docs_scene0() {
        ...
        let cands = corpus.candidate_docs("user_evidence__scene_0_q_0");
        assert_eq!(cands.len(), 3, "scene_0 has 3 candidates");
        assert!(cands.contains(&"user_evidence__scene_0_session_1_turn_1".to_owned()));
        assert!(cands.contains(&"user_evidence__scene_0_session_1_turn_3".to_owned()));
        let _ = std::fs::remove_dir_all(&dir);
    }
    ...
    fn candidate_docs_scene1() {
        ...
        let cands = corpus.candidate_docs("user_evidence__scene_1_q_0");
    ...
    fn candidate_docs_unknown_returns_empty() {
        ...
        assert!(corpus.candidate_docs("user_evidence__scene_999_q_0").is_empty());
        let _ = std::fs::remove_dir_all(&dir);
    }
    ...
    fn relevant_docs_scene0() {
        ...
        let rel = corpus.relevant_docs("user_evidence__scene_0_q_0");
        assert_eq!(rel.len(), 2, "scene_0_q_0 has 2 relevant docs");
        assert!(rel.contains("user_evidence__scene_0_session_1_turn_1"));
        assert!(rel.contains("user_evidence__scene_0_session_1_turn_3"));
        assert!(!rel.contains("user_evidence__scene_0_session_1_turn_2"), "turn_2 is not relevant");
        let _ = std::fs::remove_dir_all(&dir);
    }
    ...
    fn relevant_docs_scene1() {
        ...
        let rel = corpus.relevant_docs("user_evidence__scene_1_q_0");
        assert_eq!(rel.len(), 1);
        assert!(rel.contains("user_evidence__scene_1_session_1_turn_1"));
        let _ = std::fs::remove_dir_all(&dir);
    }
    ...
    fn relevant_docs_unknown_returns_empty() {
        ...
        assert!(corpus.relevant_docs("user_evidence__scene_999_q_0").is_empty());
        let _ = std::fs::remove_dir_all(&dir);
    }
    ...
    // ── lmeb_corpus_digest tests ─────────────────────────────────────────────

    // Fixture file contents for the cross-port golden pin.
    // Every file ends with a trailing newline. Lines separated by \n only. No BOM.
    const PIN_CORPUS: &str = concat!(
        "{\"id\":\"d0\",\"text\":\"alpha\",\"title\":\"A\"}\n",
        "{\"id\":\"d1\",\"text\":\"beta\",\"title\":\"B\"}\n",
    );
    const PIN_QUERIES: &str = "{\"id\":\"q0\",\"text\":\"who is alpha\"}\n";
    const PIN_CANDIDATES: &str =
        "{\"scene_id\":\"s0\",\"candidate_doc_ids\":[\"d0\",\"d1\"]}\n";
    const PIN_QRELS: &str = "q0\td0\t1\n"; // real tab character, \n line ending

    /// Helper: writes the four golden-pin fixture files under {dir}/user_evidence/.
    fn write_pin_fixture(base_dir: &Path) {
        write_sample_files(
            base_dir,
            "user_evidence",
            PIN_CORPUS,
    ...
    /// Cross-port golden pin (a): identical fixture → identical digest in both ports.
    /// cross-port pin — this literal also appears in LMEBCorpusDigestTests.swift;
    /// if you change one you must change the other, and if they ever disagree the ports have diverged.
    #[test]
    fn digest_cross_port_golden_pin() {
        let dir = make_test_dir("golden_pin");
        write_pin_fixture(&dir);
        let digest = lmeb_corpus_digest(&dir, &["user_evidence"]);
        let _ = std::fs::remove_dir_all(&dir);
        assert_eq!(
            digest,
            "b8702013f1d2aecf31110b3c034f58809c6a447fa2bb6e163bfd64929dc4c240",
            "cross-port golden pin mismatch — update both Rust and Swift literals together"
        );
    }

    /// (b) Content sensitivity: different corpus.jsonl content → different digest.
    /// This is the case that the old code failed (it hashed a non-content sentinel).
    #[test]
    fn digest_content_sensitivity() {
        let dir = make_test_dir("content_sens");
        // Corpus A: original pin fixture.
        write_pin_fixture(&dir);
        let digest_a = lmeb_corpus_digest(&dir, &["user_evidence"]);

        // Corpus B: same layout, different corpus.jsonl content.
        std::fs::write(
            dir.join("user_evidence").join("corpus.jsonl"),
            "{\"id\":\"d0\",\"text\":\"completely different\",\"title\":\"X\"}\n",
        )
        .unwrap();
        let digest_b = lmeb_corpus_digest(&dir, &["user_evidence"]);
        let _ = std::fs::remove_dir_all(&dir);

        assert_ne!(
            digest_a, digest_b,
            "different corpus.jsonl content must produce different digests"
        );
        // Neither digest should be "unknown" — both corpora are fully readable.
        assert_ne!(digest_a, "unknown");
        assert_ne!(digest_b, "unknown");
    }

    /// (c) Explicit failure: a missing required file produces the literal "unknown".
    #[test]
    fn digest_explicit_failure_missing_file() {
        let dir = make_test_dir("missing_file");
        write_pin_fixture(&dir);
        // Remove qrels.tsv — one of the four required files.
        std::fs::remove_file(dir.join("user_evidence").join("qrels.tsv")).unwrap();
        let digest = lmeb_corpus_digest(&dir, &["user_evidence"]);
        let _ = std::fs::remove_dir_all(&dir);
        assert_eq!(
            digest, "unknown",
    ...
    /// (d) Empty evidence-type list produces "unknown".
    #[test]
    fn digest_empty_evidence_types() {
        let dir = make_test_dir("empty_et");
        let digest = lmeb_corpus_digest(&dir, &[]);
        let _ = std::fs::remove_dir_all(&dir);
        assert_eq!(digest, "unknown", "empty evidence-type list must return 'unknown'");
    }

    /// (e) Multi-type order independence: same two types in different order → same digest.
    #[test]
    fn digest_multi_type_order_independence() {
        let dir = make_test_dir("multi_type");
        // Populate two evidence types with distinct content.
        write_sample_files(
            &dir, "evidence_a",
            "{\"id\":\"da\",\"text\":\"alpha doc\",\"title\":\"A\"}\n",
            "{\"id\":\"qa\",\"text\":\"alpha query\"}\n",
            "{\"scene_id\":\"sa\",\"candidate_doc_ids\":[\"da\"]}\n",
            "qa\tda\t1\n",
        );
        write_sample_files(
            &dir, "evidence_b",
            "{\"id\":\"db\",\"text\":\"beta doc\",\"title\":\"B\"}\n",
            "{\"id\":\"qb\",\"text\":\"beta query\"}\n",
            "{\"scene_id\":\"sb\",\"candidate_doc_ids\":[\"db\"]}\n",
            "qb\tdb\t1\n",
        );

        let digest_ab = lmeb_corpus_digest(&dir, &["evidence_a", "evidence_b"]);
        let digest_ba = lmeb_corpus_digest(&dir, &["evidence_b", "evidence_a"]);
        let digest_a_only = lmeb_corpus_digest(&dir, &["evidence_a"]);
        let digest_b_only = lmeb_corpus_digest(&dir, &["evidence_b"]);
        let _ = std::fs::remove_dir_all(&dir);
