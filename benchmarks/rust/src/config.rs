...
//!     terse config still decodes and runs against the MOOTx01/external-server
//!     defaults.
...
pub enum ResultFormat {
    ...
    /// no stable id, e.g. a search-only endpoint), `content_key` names the content.
    JsonObjects {
    ...
    MootText,
    /// ARIA v2 structured results — reads `structuredContent.data` envelope.
    /// Handles search (results[].memory_id), recall lenses (results[].id),
    /// write receipts (data.memory_id), and memory-get (memories[].memory_id).
    /// Falls back to text_blocks for `discrimination: low` detection.
    MootV2,
}
...
impl VerbMap {
    ...
    /// with defaults — used by the endpoint-config tests.
    ...
    pub fn new(
        ...
        VerbMap {
            ...
            // unaffected by the constructor constraint (new() signature unchanged).
            dense_query: None,
...
/// `["location": "import/external-source"]` default.
fn default_constant_args() -> BTreeMap<String, String> {
    let mut m = BTreeMap::new();
    m.insert("location".to_string(), "import/external-source".to_string());
    m
}
...
mod tests {
    ...
    fn valid_config_loads() {
        ...
        assert_eq!(cfg.source.name, "external-source");
    ...
    fn terse_verb_map_defaults_apply() {
        ...
        assert_eq!(
            ...
            Some("import/external-source")
        );
        assert_eq!(vm.result_format, ResultFormat::default_format());
    }

    #[test]
    fn external_source_format_decodes() {
        ...
        assert_eq!(vm.list.as_deref(), Some("external_list_items"));
        assert_eq!(
            vm.result_format,
            ResultFormat::JsonObjects {
    ...
    fn moot_text_format_decodes() {
        ...
        // v2: tool names now come from aria_v2_surface constants; verify the
        // decoded VerbMap carries the same string the adapter exports.
        assert_eq!(vm.write, crate::aria_v2_surface::FILE_MEMORY);
        assert_eq!(vm.query, crate::aria_v2_surface::MEMORY_SEARCH);
    ...
    #[test]
    fn external_two_constant_args() {
        // An external MCP server whose write tool requires two routing args
        // (wing + room) rather than one — proves multi-constant generalization.
        let vm = load_verb_map(
            r#"{
              "write": "external_add_item",
              "query": "external_search",
...
/// seconds. One shared bound: the judge, the reranker, and the offline answer
/// command are the same class of external command (prompt on stdin, reply on
/// stdout) and must time out identically. The default 120 s accommodates a
/// slow local model on an idle machine; `MOOT_BENCH_SUBPROCESS_TIMEOUT` (whole
/// seconds, > 0) raises it when one local server is shared by several lanes
/// at once. A hung command past this bound is killed, never awaited. Twin of
/// Swift `subprocessTimeoutSeconds`.
pub fn subprocess_timeout_secs() -> u64 {
    std::env::var("MOOT_BENCH_SUBPROCESS_TIMEOUT")
        .ok()
...
/// The canonicalized system temp base. On macOS `/tmp` is a symlink to
/// `/private/tmp`, so comparing a CANONICALIZED scratch path against a
/// literal "/tmp/…" prefix rejects every legitimate directory (Wave-3 G3).
/// All scratch-path containment checks build their expected prefix from
/// THIS value instead of a hardcoded "/tmp/" literal. Falls back to the
/// literal only if /tmp itself cannot be resolved.
pub fn canonical_tmp_base() -> std::path::PathBuf {
    std::fs::canonicalize("/tmp").unwrap_or_else(|_| std::path::PathBuf::from("/tmp"))
}

/// Bounded reap for judge/reranker children: poll `try_wait` until
/// `deadline`, then kill and reap. A child that closes both of its pipes
/// but keeps running (`exec 1>&- 2>&-; sleep …`) satisfies the pipe-drain
/// timeout yet would hang a plain `wait()` forever — the drain threads see
/// EOF immediately while the process lives on. Returns None when the
/// deadline expired (child killed) or the wait errored.
    ...
    loop {
        match child.try_wait() {
            ...
            Ok(None) => {
                ...
                // 20ms poll: coarse enough to cost nothing against a 120s
                // budget, fine enough that a normal exit reaps promptly.
