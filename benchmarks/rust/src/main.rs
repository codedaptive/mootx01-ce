...
fn usage() -> &'static str {
    "mcp-benchmarker-rs — Rust twin of the mcp-benchmarker LME/LoCoMo core\n\
     ...
     \x20\x20                               [--run-id <id>]\n\
     \x20\x20mcp-benchmarker-rs artifact-recall --dataset locomo|convomem|membench|lme-s [--chest-diversity on|off]\n\
...
fn run_artifact_recall(args: &[String]) -> Result<(), String> {
    ...
    let chest_diversity = match option_value("--chest-diversity", args) {
        None => None,
        Some("on") => Some("on".to_string()),
        Some("off") => Some("off".to_string()),
        Some(other) => return Err(format!("--chest-diversity must be 'on' or 'off'; got '{other}'")),
    };
