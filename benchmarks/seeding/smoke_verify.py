...
# estate is RED once the corpus lane has covered every drawer: the span lane
# runs behind the corpus lane on the same encoder, so while corpus coverage
# is still short (a 19k-drawer seed such as the lme-s aggregate is still
# corpus-encoding when a probe window closes, measured 2026-09-18: 13,072 of
# 19,195 indexed and no span yet at 1200 s) zero spans is the expected state
# and the check is informational, like encode coverage itself.
span_bounded = args.partial or args.resumable
corpus_short = span_bounded and n_indexed < n_drawers
if corpus_short:
    span_ok = n_span_covered <= n_eligible
elif span_bounded:
    # Corpus lane complete and no span: the span lane never started. RED.
...
check("span coverage matches eligible drawers",
      ...
      f"eligible drawers"
      + (f" (corpus lane at {n_indexed}/{n_drawers}; span lane follows — not gated)"
         if corpus_short else ""))
