...
## 9. Make targets
    ...
    make stash-wing-estate DS= PORT= EXTERNAL=   move a built wing estate out to an external store's wing-store/
    make restore-wing-estate DS= PORT= EXTERNAL= copy it back under the wing root for a run
...
`stash-wing-estate` / `restore-wing-estate`: a built Form-2 wing estate moves
to `<external store>/wing-store/<port>/<ds>` and back the way a fleet does,
with no catalog to rewrite; the stash refuses while the estate's PID marker
names a live resident.
