...
def lay_out_dataset(target: TargetMap, port: str, dataset: str,
    ...
    Resume: a unit whose address is already past laid_out (a bounded build,
    or a build that stopped) keeps its estate and its state; the set plan is
    a pure function of the sorted unit ids, so a kept unit sits in the same
    set it was built in. The aggregate is the union of every unit, so when
    any unit is new the aggregate is laid out again from nothing and its
    partition maps are dropped; when no unit is new it is kept too. Every
    later step skips what is kept (`provision_all`, `import_waves`); the
    watcher already resumes from the first estate not yet encoded.
    """
    ...
    new_units = len(unit_ids) - kept
    if is_built(adir) and new_units == 0:
        pass                                   # complete build: the aggregate stands
    else:
        if adir.exists():
            shutil.rmtree(adir)                # a partial aggregate is worthless
        for stale in adir.parent.glob("partition_map*"):
            stale.unlink()
...
def is_built(estate: pathlib.Path) -> bool:
    """True when the estate's address is past laid_out and not failed: the
    unit was provisioned, imported or encoded by an earlier run and a resume
    keeps it."""
    path = estate / "address.json"
    if not path.exists():
        return False
    state = read_address(estate).get("state")
    return state in ("provisioned", "imported", "encoded")
