...
class ResumeTests(WatchTests):
    ...
    def test_resume_retires_a_done_map_when_a_set_gains_units(self):
        """A set the watcher finished keeps partition_map.done.json. A wider
        resume lays a new unit into that set; the done marker must be
        retired and a fresh claimable map written, or the new unit never
        drains (the watcher reads the done marker as terminal)."""
        first = self.build(n_units=3, set_size=2, records=2)      # set1: u0,u1  set2: u2
        for s in first:
            if s.name == L.AGGREGATE_SET:
                continue
            (s.path / "partition_map.json").rename(s.path / "partition_map.done.json")
            for e in s.estates.values():
                addr = L.read_address(e)
                L.write_address(e, unit=addr["unit"], dataset="demo", port="swift",
                                set_name=s.name, state="encoded", record_count=addr["record_count"])
        proj = self.root / "proj-wider"
        units = {f"u{i}": [rec(f"u{i}r{j}") for j in range(2)] for i in range(4)}
        write_projection(proj, units)
        projection = I.Projection.open(proj)
        target = L.TargetMap([L.BaseFolder(self.bases[0], 10**9), L.BaseFolder(self.bases[1], 10**9)], 0)
        sets = L.lay_out_dataset(target, "swift", "demo", projection.unit_ids(),
        ...
        for s in sets:
            if s.name == L.AGGREGATE_SET:
                continue
            widened = any(u == "u3" for u in s.estates)
            if widened:
                self.assertFalse((s.path / "partition_map.done.json").exists(),
                                 "the done marker is retired once the set holds an unbuilt unit")
                self.assertTrue((s.path / "partition_map.json").exists(),
                                "a fresh claimable map covers the widened set")
                self.assertTrue(any(p.name.startswith("partition_map.done.") for p in s.path.iterdir()),
                                "the old done map is kept aside, not deleted")
            else:
                self.assertTrue((s.path / "partition_map.done.json").exists(),
                                "a set with no new unit keeps its done map")
                self.assertFalse((s.path / "partition_map.json").exists())
