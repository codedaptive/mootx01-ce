...
/// (any corpus without has_answer): absence of evidence text is not
/// evidence of a hit.
...
///
/// Returns nil when no `has_answer` turns are present. The fetched cleaned
/// fixtures (longmemeval_s_cleaned.json) carry the field; a corpus without it
/// yields nil, meaning the evidence-hit score is unavailable for this
...
// MARK: - Gold-answer reachability (judged-run diagnostic)

/// Whether the question's GOLD ANSWER text appears in the payload the judge
/// was given.
///
/// This is the retrieval ceiling for the answer-accuracy metric: if the gold
/// answer never reaches the judge's context, no judge can answer the question,
/// and accuracy is bounded by retrieval rather than by the judge's ability.
/// Comparing this rate against answer accuracy separates the two:
///
///   gold reachable HIGH, accuracy LOW  → the judge is the ceiling
///   gold reachable LOW                 → retrieval is the ceiling
///
/// Distinct from `lmeEvidenceHit`, which needs the corpus's `has_answer` turn
/// annotations. On a corpus without that field the evidence rate is
/// unavailable (nil for absence of annotation, NOT for absence of evidence).
/// The gold answer is always present in the dataset, so this diagnostic works
/// on every corpus.
///
/// CAVEAT — short gold answers over-report. A gold answer of "5" is a
/// substring of almost any payload, so a hit does not prove the right "5" was
/// found. Read this rate as an UPPER BOUND on retrieval, not a recall score.
func lmeGoldAnswerInPayload(goldAnswer: String, payloadText: String) -> Bool {
    let normalizedGold = lmeNormalizeForEvidence(goldAnswer)
    guard !normalizedGold.isEmpty else { return false }
    return lmeNormalizeForEvidence(payloadText).contains(normalizedGold)
}
