...
struct ArtifactRecallConfig: Sendable {
    ...
    /// nil sends nothing and the estate preference decides.
    let chestDiversity: String?
}
...
func askArtifactQuestions(
    ...
    for question in questions {
        ...
        if let chestDiversity = config.chestDiversity {
            args["chest_diversity"] = .string(chestDiversity)
        }
...
private func artifactRecallReport(
    ...
    return [
        "config": [
            ...
            "chest_diversity": config.chestDiversity ?? "",
        ],
        "n_questions": n,
        "n_units": unitCount,
...
func runArtifactRecall(_ args: [String]) async throws {
    ...
    // --chest-diversity on|off: the per-call ARIA global modifier on every
    // search (ADR-027 D3); absent, the estate preference decides.
    let chestDiversity = optionValue("--chest-diversity", in: args)
    if let chestDiversity, chestDiversity != "on", chestDiversity != "off" {
        throw MCPError(description: "--chest-diversity must be 'on' or 'off'; got '\(chestDiversity)'")
    }
