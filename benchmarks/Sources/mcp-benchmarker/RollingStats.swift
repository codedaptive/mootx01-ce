...
// long-running run (the `pressure` driver and other streaming callers).
//
// Unlike `TimingCollection` (which a one-shot benchmark builds and then
// summarizes once), these stats accumulate continuously as traffic flows.
// A streaming caller updates them on every call; the pressure driver updates
// them per op per path. A snapshot can be taken at any time to render a report
// or to emit the current state into the ObserverSink stats store.
//
// Concurrency: a streaming caller may feed calls serially on one reader loop,
// but fan-out and the pressure driver run concurrent Tasks, so the
...
public struct RollingSeriesSnapshot: Sendable {
...
public actor RollingStats {
    ...
    /// series on first use.
    public func recordLatency(_ seconds: Double, label: String) {
    ...
    public func recordDivergence(jaccard: Double, kendallRank: Double) {
        jaccardSum += jaccard
    ...
    public func snapshot() -> RollingStatsSnapshot {
...
extension RollingStatsSnapshot {
    ...
    /// Public so out-of-package callers can render rolling-stats heartbeats to
    /// stderr using the same format.
