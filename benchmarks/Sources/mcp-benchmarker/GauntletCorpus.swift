...
public enum NoiseTier: String, Codable, Sendable, CaseIterable, Comparable {
    case lexical = "T1"
    case semantic = "T2"
    case temporal = "T3"
    case split = "T4"
    ...
    public static func < (lhs: NoiseTier, rhs: NoiseTier) -> Bool {
...
public enum RecordRole: String, Codable, Sendable, Equatable {
    case needle
    case distractor
    case splitPartner
}
...
///
/// `location` is the per-record filing target. Every backend files by location:
/// mootx01's `moot_file_memory` takes the single `location` path verbatim; an
/// injected baseline maps it through its own write-argument closure (see
/// `GauntletBaseline.writeArgs`), so one field drives every backend and the T5
/// scatter tier can place a record anywhere by setting this string.
public struct GauntletRecord: Codable, Sendable, Equatable {
    ...
    /// The filing location, `wing/room[/...]` form. Passed whole as the
    /// mootx01 location; a baseline's write-argument closure derives its own
    /// filing fields from it.
    ...
    public let role: RecordRole
    /// The id of the needle this record orbits (a needle points at itself).
    public let needleID: String

    public init(id: String, content: String, location: String,
                tier: NoiseTier, role: RecordRole, needleID: String) {
        self.id = id
...
public struct Needle: Codable, Sendable, Equatable {
    ...
    public let distractorIDs: [String]
    ...
    public let splitPartnerID: String?
...
public struct GauntletCorpus: Sendable {
    public let seed: UInt64
    public let records: [GauntletRecord]
