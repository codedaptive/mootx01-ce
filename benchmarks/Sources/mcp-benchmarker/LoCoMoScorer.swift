...
struct LoCoMoReportPerQuestion: Codable, Sendable {
    ...
    /// Verbatim payload text (joined textBlocks) the recall verb returned for this
    /// question. Persisted so retrieval diagnosis (which chunks came back, whether
    /// a bridge query fired) reads from the report instead of requiring a re-run
    /// with instrumentation. Nil when the MCP response carried no textBlocks.
    /// Reports live in gitignored results directories, so size is a disk concern
    /// only, never a repo concern.
    let payloadText: String?
...
struct LoCoMoReport: Codable, Sendable {
    ...
    var latency: LoCoMoReportLatency? = nil
    ...
    /// Query verb used for recall calls: "search" (default, moot_memory_search),
    /// "shaped" (moot_recall_shaped), or "precise" (moot_recall_precise).
    /// All three strategy cells are distinguishable by this field and the run label.
    let recallStrategy: String
    /// Named RecallShape preset when recallStrategy is "shaped". nil when the
    /// strategy is not "shaped" or when the product's default preset was used.
    let recallShape: String?
    // MARK: Rerank (additive — W2-rerank)
    /// Whether --rerank-cmd was supplied. True = a rerank command was active for
    /// this run. PRESENCE ONLY — the command text carries API keys and is never
    /// recorded (secrecy law, same as judge_cmd).
    let rerankCmdSet: Bool
    /// Number of questions where the rerank command returned a non-zero exit or an
    /// unparseable reply. nil when --rerank-cmd was absent (field not applicable).
    ...
    // MARK: Estate shape + parallelism (additive — C1/C6, benchmark reset 2026-08-13)
    /// Backend persistence shape used for scratch estates: "disk" (SQLite, default)
    /// or "ram" (in-memory via --in-memory). Additive — existing reports
    /// without this field decode cleanly via a default of "disk".
    let shape: String
    /// Effective conversation concurrency. Internal only — never emitted:
    /// a run is a run; width is not a property of accuracy figures.
    ...
    /// The fleet topology used for this run. Always "per-conversation": one
    /// restored artifact per conversation, the published LoCoMo protocol. The
    /// key is kept so downstream report parsers keep working. Byte-identical
    /// field name across Swift and Rust ports.
    let estateShape: String
    /// True when a run departs from the published per-conversation protocol.
    ...
    enum CodingKeys: String, CodingKey {
        ...
        case granularity
        case recallStrategy    = "recall_strategy"
...
func buildLoCoMoReport(
    ...
    // C1/C6 additions: shape and effective parallel unit count.
    // Passed explicitly so the builder does not reach into LoCoMoRunConfig fields
    // that the caller already resolved (parallelConversations has a computed default
    // that the CLI may override — pass the resolved value, not the config field).
    shape: String? = nil,
    parallelUnits: Int? = nil,
    ...
    // Estate topology: every run is one restored artifact per conversation —
    // the published protocol. The report keys stay for downstream parsers;
    // their values are constants of the lane.
    let estateShape = "per-conversation"
    ...
    return LoCoMoReport(
        ...
        // Lane constant (retired axis); key kept for report parsers.
        granularity: "turn",
        recallStrategy: config.strategy.rawValue,
        recallShape: config.recallShape,
        // Rerank presence only (secrecy law): command text never recorded.
        ...
        // C1/C6: shape and parallel concurrency level.
        shape: shape ?? config.shape.rawValue,
        parallelUnits: parallelUnits ?? config.parallelConversations,
