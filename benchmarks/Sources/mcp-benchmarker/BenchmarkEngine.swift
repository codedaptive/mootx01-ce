...
//
// CROSS-SERVER ID ASYMMETRY: the source and target mint ids in disjoint
// spaces — a source's search may return no id at all, and the target assigns
// its own UUID on write. So source-vs-target rank
...
// probe so the report shows source recall latency alongside the target's.

/// Verifies a recorded manifest against the target and produces the benchmark
/// report.
public struct BenchmarkEngine {
    ...
    func run(manifest: Manifest) async throws -> BenchmarkReport {
        ...
        for query in queries {
            ...
            if compareSourceRanking {
                // Time the source query for its recall latency, then
        ...
        return BenchmarkReport(
            ...
            // the source server's recall latency, sitting
    ...
    /// formatting differences (e.g. a source's truncated previews vs full
    /// MOOTx01 content) on the shared prefix that survives both.
    public static func normalizedContentOrder(_ items: [MCPResultItem]) -> [String] {
        items.compactMap { item in
            ...
            // (a preview-only source result shape) still matches the same item
            // from a server that returns it in full.
