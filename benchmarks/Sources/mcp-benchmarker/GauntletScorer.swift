...
//
// IDENTITY ACROSS BACKENDS. Backends do NOT share an id space, and a baseline
// product's search may return NO id at all — only content. So scoring
...
// collapse whitespace, bounded prefix). Content is the only identity every
// backend exposes for a search hit. An id, when present, is retained for
...
public struct ScoredItem: Sendable, Equatable {
...
public struct NeedleScore: Sendable, Equatable {
    ...
    public var reciprocalRank: Double { rank.map { 1.0 / Double($0) } ?? 0.0 }

    public init(needleID: String, tier: NoiseTier, foundAtK: [Int: Bool], rank: Int?,
                completeness: Double, contamination: Int,
                latencySeconds: Double, bytesReturned: Int) {
        self.needleID = needleID
        self.tier = tier
        self.foundAtK = foundAtK
        self.rank = rank
...
public struct GauntletScorer: Sendable {
    ...
    /// tool uses. (The prefix bound absorbs a source's preview truncation vs
    ...
    ///     by content match — the only key an id-less baseline exposes).
    ...
    public func score(needle: Needle,
               returned: [ScoredItem],
