...
// MARK: - Guard sampling policy

/// Controls how often the DegeneracyGuard probe is issued within one benchmark leg.
///
/// The guard validates a property of the BINARY under test, not of any individual
/// unit (question, query, or item). Probing on the first unit and caching the
/// verdict for the remainder of the leg is correct and cuts 3 × (N-1) unnecessary
/// MCP search calls from an N-unit leg.
///
/// Use `.perUnit` only when debugging a suspected per-estate guard failure —
/// it restores the original all-units behavior at the cost of 3 extra MCP calls
/// per unit.
public enum GuardSamplingPolicy: String, Sendable {
    /// Run the guard probe on the first unit of a leg only (default).
    ///
    /// The verdict is cached and propagated to all subsequent units.
    /// A refusal on the first unit marks every subsequent unit unhealthy too,
    /// so the leg fails loudly across its full result set.
    case oncePerLeg = "once"

    /// Run the guard probe on every unit.
    ///
    /// Retains the original per-unit behavior. Use for debugging only —
    /// not for production runs.
    case perUnit = "per-unit"

    /// Parses a `--guard-sample` CLI value; nil input = default (`.oncePerLeg`).
    /// Fails closed on unknown values.
    static func parse(_ raw: String?) throws -> GuardSamplingPolicy {
        guard let raw else { return .oncePerLeg }
        guard let policy = GuardSamplingPolicy(rawValue: raw) else {
            throw MCPError(description:
                "--guard-sample must be \"once\" or \"per-unit\"; got '\(raw)'")
        }
        return policy
    }
}

// MARK: - Leg-level guard sampler

/// Manages DegeneracyGuard probe execution across the units of one benchmark leg.
///
/// Create one sampler at the start of a leg; pass its `probe(using:)` method into
/// each unit's run logic. The sampler enforces the leg's `GuardSamplingPolicy`:
///
/// - `.oncePerLeg` (default): the prober closure runs on the FIRST unit only;
///   all subsequent units receive the cached verdict with no MCP calls issued.
///   A refusal verdict is cached and propagated to every subsequent unit,
///   so the leg fails loudly across the full result set.
/// - `.perUnit`: the prober closure runs on every unit, matching the original
///   behavior before sampling was introduced.
///
/// Thread-safety: this type is an actor — safe for concurrent callers, though
/// the benchmarker's sequential per-unit loop does not require that.
public actor LegGuardSampler {

    /// The sampling policy this sampler enforces.
    public let policy: GuardSamplingPolicy

    /// True once the first probe has completed.
    private var probed = false
    /// The verdict from the first (and, in `.oncePerLeg` mode, only) probe.
    private var cachedVerdict: DegeneracyGuard.Verdict = .healthy

    public init(policy: GuardSamplingPolicy = .oncePerLeg) {
        self.policy = policy
    }

    /// Executes or skips the guard probe for one unit.
    ///
    /// - `prober`: async closure that issues MCP probe queries and returns the
    ///   raw probe rankings. Called only when a real probe is warranted by the
    ///   policy.
    /// - Returns: `(verdict, wasProbed)` where `wasProbed` is `true` when
    ///   `prober` was called and `false` when the cached first-unit verdict was
    ///   returned without issuing any MCP calls.
    public func probe(
        using prober: @Sendable () async -> [[String]]
    ) async -> (verdict: DegeneracyGuard.Verdict, wasProbed: Bool) {
        switch policy {
        case .perUnit:
            let rankings = await prober()
            return (DegeneracyGuard().classify(probeRankings: rankings), true)
        case .oncePerLeg:
            if probed {
                // Return the cached verdict from the first unit — no MCP calls.
                return (cachedVerdict, false)
            }
            let rankings = await prober()
            let verdict = DegeneracyGuard().classify(probeRankings: rankings)
            probed = true
...
public struct DegeneracyGuard: Sendable {
    ...
    public func classify(probeRankings: [[String]]) -> Verdict {
    ...
    /// is above `confirmationRecallFloor`. This is the confirmation
    /// round-trip defect pattern observed in a source product: the count says nothing was confirmed, but the
    ...
    /// "found N" with N > 0. In ARIA_MCP_SPEC 2.0.0 the header is
    /// "found N candidate memories, one per line"; the scan only needs the
    /// second word to be a positive integer, so it works for both formats.
    private func containsFoundPositive(in text: String) -> Bool {
        // Pattern: scan for "found <positive-integer>" regardless of what follows.
