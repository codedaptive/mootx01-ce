...
//   3. Maintain a fixed-size min-heap of the K smallest distances.
...
public enum HammingNN {
    ...
    public static func topK<S: Sequence>(
        ...
        for (rowID, fingerprint) in candidates {
            ...
            if heap.count < k {
                heap.append(HammingNNHit(rowID: rowID, distance: d))
                heapifyUp(&heap)
            } else if d < heap[0].distance {
                heap[0] = HammingNNHit(rowID: rowID, distance: d)
                heapifyDown(&heap)
            }
        }
    ...
    // MARK: - Max-heap helpers (root has largest distance)
