...
public enum Hamming {
    ...
    /// uses all four blocks (full 256-bit distance). Restrict to
    /// `blocks` for per-aspect distance.
    ///
    /// Returns an integer in [0, 64 * blocks.count]. For the full
    ...
    public static func similarity(_ a: Fingerprint256,
                                   _ b: Fingerprint256,
                                   blocks: Set<Int> = [0, 1, 2, 3]) -> Double {
        let max = 64 * blocks.count
