...
public struct MatrixF: Sendable, Equatable {
    ...
    public static let cellCount = fieldCount * bitsPerField  // 216
    ...
    ///
    /// `bitPresence(field:bit:)` returns true if the row has that
    /// (field, bit) set after decoding the relevant bitmap column.
    public mutating func applyRow(delta: Int64,
                                   bitPresence: (Int, Int) -> Bool) {
        guard delta != 0 else { return }
        for field in 0..<Self.fieldCount {
            for bit in 0..<Self.bitsPerField {
                if bitPresence(field, bit) {
