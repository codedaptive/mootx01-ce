...
public struct BnnsKernel: SubstrateKernel {
    ...
    public func hammingTopK(probe: Fingerprint256,
        ...
        // distance computation, then sort + truncate on CPU.
        // Tie-breaking is by candidate index, matching the
        // scalar contract.
        let distances = hammingDistanceBatch(probe: probe, candidates: candidates)
        var scored: [(index: Int, distance: Int)] = []
        scored.reserveCapacity(distances.count)
        for (idx, dist) in distances.enumerated() {
            scored.append((index: idx, distance: dist))
        }
