...
# Substrate Conformance Harness Reference — v1.0

## §0. Read me first (agents, this is your TL;DR)

If you are an LLM agent — Claude Code, Cursor, Cowork, a custom
substrate orchestrator — and you are about to write a SimHash, a
...
(`GENIUSLOCUS_ENGINEERING_COOKBOOK_v1.0_2026-05-28.md`); this doc
...
- **What:** Fowler-Noll-Vo 1a string hash family — the substrate's
  ...
  tokenization. Promoted to gated primitive 2026-05-28 (F5b).
...
- **What:** Masked field-equality predicate, `(bitmap & mask) == expected`,
  over packed-row Int64 bitmaps (cookbook §2.8 / §7.7). The F18
  atomic-centralization primitive that kit-level field-equality
  checks route through: LocusKit's `andMask` is a one-line
  ...
  across ports. Promoted to gated primitive 2026-05-29 (F18.2b).
...
#### `matrix_decay` — §6.8 — CRC `0x7b12f93d`  *(promoted 2026-05-28)*
...
#### `moment_summary` — §8.7 — CRC `0x6762440b`  *(promoted 2026-05-28)*
...
#### `field_presence_matrix_f` — §6.1 — CRC `0x2a051f09`  *(promoted 2026-05-28)*
...
#### `eigenvalue_centrality` — §7.2 — CRC `0x1a9039ea`  *(promoted 2026-05-28)*
...
- `community_detection` phase 2 (Louvain phase 2 graph aggregation)
  — cookbook §7.3. **Deferred to v0.37.** Do NOT promote yet.
...
| AuditEvent | `eventID: UUID` (in `SubstrateTypes/AuditEvent.swift`, Phase 6.6) | `event_id: u128` (deterministic content-ID per `audit_gate::content_id`, in `SubstrateTypes/rust/src/audit_event.rs`) | Both ports' audit-log-fold algorithms ignore the field; output is independent of it. |
...
1. **Decide what you are gating.** A primitive is a pure function
   ...
   `packages/libs/SubstrateML/Sources/SubstrateML/` (post-Phase-6
   split; see `docs/validation/substrate_math_performance/test-harness/primitive-catalog.md`
...
- `AuditEvent` (with `event_id`/`eventID` once the Swift gap closes)
...
- The `combinators` layer: `zip4`/`reduce4`/`map4`/`popcount`
  over Fingerprint256 (added in refactor Phase 1)
...
1. **Swift `AuditEvent.eventID` parity.** [RESOLVED in Phase 6.6,
   2026-05-29] Both ports now expose the field. Swift `AuditEvent`
   in `packages/libs/SubstrateTypes/Sources/SubstrateTypes/AuditEvent.swift`
   exposes `eventID: UUID`; Rust `AuditEvent` in
   `packages/libs/SubstrateTypes/rust/src/audit_event.rs` exposes
   `event_id: u128`. The substrate-lib symmetry is no longer
   asymmetric on this field. `audit_log_fold` continues to ignore
   the field; its CRC (`0xa747722e`) is unaffected.

2. **`community_detection` phase 2.** Deferred to v0.37 per
   cookbook §7.3 (Louvain phase 2 graph aggregation). Do not
   promote until v0.37 release.

3. **Wikidata graph distance** (the QID half of lattice distance).
   Requires a remote adjacency provider; not currently gated.
   `lattice` covers only the UDC-tree half (the deterministic
   cheap part).

4. **`bitmap_filter` over the bit-slice tensor.** Cookbook §4.5 P1.
   Requires the bit-slice substrate-level work (§4.1 I-18) to land
   first. Tracked in `Phase 2.ε backlog item 2`.

5. **Composite distance (`α·lattice + β·fingerprint`).** Cookbook
   §8.4. Not yet gated. Awaits Wikidata adjacency provider gating.

6. **NMF, FFT, eigenvalue_centrality cookbook estimates.** Cookbook
   §17.1 / §17.2 budgets for these are pre-Phase-2-measurement
...
- Cookbook: `docs/engineering/GENIUSLOCUS_ENGINEERING_COOKBOOK_v1.0_2026-05-28.md`
