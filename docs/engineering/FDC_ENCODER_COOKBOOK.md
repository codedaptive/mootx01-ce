...
# FDC Encoder Engineering Cookbook
...
reads this and ships code. The canonical spec (`FDC_ENCODER_CANONICAL.md`)
...
**Writable artifact + LIVE in-session swap (not a frozen cache).** The
bundled `Resources/WordClassTable.json` is the pristine seed. The
PoolReducer (§2.2) merges novel-token observations into a *writable*
copy of the table at the pool directory's sibling path
(`…/lattice/WordClassTable.json`). The running tagger reads the table
through a process-wide, **live-swappable** holder
(`WordClassTableCache`, Swift; `GLOBAL_TABLE = RwLock<Arc<…>>`, Rust):

- **Load precedence**: at process start the holder is seeded
  writable-first (the merged artifact if present, else the bundled
  seed).
- **Live atomic swap**: after a reduce merges novel tokens, the
  resident Autonomic Governor publishes the new table into the holder
  at a post-reduce safe point — a single locked publish of a new
  immutable snapshot plus a monotonic version bump, with no torn reads
  (a reader copies the whole snapshot reference out under the lock,
  then tests membership outside it). The **running** tagger adopts the
  merged tokens on its very next `wordClass` call — **no process
  restart**. This supersedes the earlier process-frozen-cache note.

Tagging stays deterministic given **(input, table-version)**: for a
fixed table version the same token yields the same `WordClass` on every
platform for any table-resident token; a live swap advances the version.
...
**Reduce + live swap cadence — NEAR-REALTIME (not hourly).** The
resident Autonomic Governor drives the PoolReducer
**near-realtime**: it is considered on every governor tick (default
`MOOTX01_POOL_REDUCE_CADENCE_SECONDS = 0`), gated cheaply by the
...
supersedes the earlier hourly-cadence framing.

**QueueKit near-realtime ingest+classify (regardless of load/sequence).**
Captured content is enqueued instantly onto a per-estate QueueKit and
classified/encoded near-realtime by a background drain worker:
observer-`watch`-driven where the concrete backend supports it (the Rust
port's background thread), or an on-actor whole-batch poll worker (the
Swift port, ~15 ms floor, drains the entire claimable batch each pass so
it never starves under burst). The queue absorbs bursts and out-of-order
arrival without dropping or wedging; the latency floor is the drain
worker's wake, not any tick. The await-drain latch is the authoritative
"encoding finished" barrier for bulk callers.
...
maintainer CLI (a maintainer-only tool). Inputs: the WordNet `dict/` index
...
**Shipped scoring — IDF-weighted.** The raw-overlap sum above
...
`FDC_ENCODER_CANONICAL.md` §5. The cutoff compares against the **raw integer
...
The Seed Generator (a maintainer-only tool) produces `FDCSignatures.json`.
...
| Seed Generator | the seed generator (maintainer-only) | No (maintainer only) |
