...
SQLCipher data key is a 256-bit key **stored in the Keychain**. This applies to
**both iOS and macOS**, unifying the product on one encryption model with two
FIPS-validated backends (OpenSSL FIPS provider on Windows/Linux, CoreCrypto on
Apple).

> **CORRECTION (2026-07-31) — what shipped differs from what this ADR proposed.**
> This document originally said the data key was *"wrapped by a Secure Enclave
> key … gated by user authentication."* **The implementation does neither**, and
> has not since the feature shipped in v1.0.35. What actually exists
> (`EstateKeyProvider.swift`, `KeychainKeyStore.swift`):
>
> - 32 bytes from `SecRandomCopyBytes` — **no KDF, no passphrase, no Secure
>   Enclave wrapping**.
> - Stored as a `kSecClassGenericPassword` item with
>   `kSecAttrAccessibleAfterFirstUnlockThisDeviceOnly` — **this-device-only, not
>   synced**, and **not** gated by LAContext or any user authentication.
> - Keychain account is `estate-db-key.<SHA-256 of the standardized estate
>   path>`, service `com.codedaptive.mootx01`, access group
>   `com.codedaptive.mootx01.shared`.
>
> Two consequences the original text obscured, both load-bearing:
>
> 1. **There is no escrow and no recovery.** Lose the Keychain item and the
>    estate is permanently unreadable. There is deliberately no decrypt path
>    (`--no-encrypt` explicitly refuses to reverse an encrypted estate).
> 2. **The key is bound to the estate's PATH.** Moving the estate file or
>    changing `MOOTX01_DATA_DIR` changes the derived account and strands the
>    key. This is the same failure class as closed finding #75/#474, which fixed
>    the *access-group* case but not the *path* case.
>
> `SECURITY.md` was corrected at the time (CE-1.0.35-09) to stop implying Secure
> Enclave custody; this archived ADR was missed and kept asserting it. Treat
> `SECURITY.md` and the source as authoritative over this document.
>
> **The per-port custody split is BY DESIGN (Bob's ruling, 2026-07-31).** The
> Apple vertical holds the key in the Keychain; the Rust vertical writes a
> sibling key file beside the estate (`aria_mcp::INSTALL_KEY_FILE`). The two
> upgrade prompts differ accordingly — Apple says the plaintext original moves
> to the Trash, Rust says it is kept beside the encrypted copy — and each is
> accurate for its own port. Do NOT "reconcile" the prompt wording: the text
> differs because the behavior differs, and equalizing the strings would
> conceal the split rather than resolve it. Do not file the sibling key file as
> a defect either; the platforms have different key stores available and this is
