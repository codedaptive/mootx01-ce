...
supersedes: none
context:
  - Bounded-additive implementation task; adds a moot_vault_* tool family to the Swift ARIA_MCP dispatch.
  - Prereq VaultKit implementation merged — VaultBridge (export/importVault) is importable.
...
future VaultKit-Rust-port implementation task. This asymmetry is documented
...
- Rust `moot_vault_*` mirror — the future VaultKit-Rust-port implementation task.
- Wiring a real QueueKit instance into ARIA_MCP so reconcile enqueues
  (leg a) instead of returning — the gated downstream implementation task that also
...
superseded.** A subsequent parity workstream (PARITY_WAVE_PROGRESS,
2026-06-05 — two days after this ADR) completed:
...
bar. Per the parity-is-absolute standing rule (Bob, 2026-06-06 — "we
wouldn't do a swift-only version"), a Swift-only kit is not a shippable end
state.
