...
**Clarification (2026-06-17, planned encryption lockdown).** The rejection above is specifically of whole-file encryption as the FEDERATION-TRANSPORT mechanism — it cannot express per-record keys and makes sharing scale with database size. It is NOT a rejection of whole-file encryption as a LOCAL at-rest structure-protection layer. The two are orthogonal: per-record encryption (Mode 2 / RowEncryption) remains the federation grant mechanism, while Mode 3 (FullDatabase) adopts each platform's native whole-file encryption — SQLCipher on the Rust port (Windows/Linux), Apple Data Protection on the Apple port (iOS) — to protect the local database structure and content from any process that does not hold the key. The layers compose, and the federation analysis here is unaffected: a federating estate uses Mode 2. See PERSISTENCEKIT_SPEC B-12.
