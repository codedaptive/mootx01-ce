...
The consumer path to the manifest is now `GeniusLocusKit.meta(in:key:)` and
`GeniusLocusKit.setMeta(in:key:value:)`, handle-scoped (GENIUSLOCUSKIT_INTERFACE
3.39.0). `GeniusLocusKit.estate(for:)` is internal to the kit and no consumer
holds a `LocusKit.Estate`. The storage ownership and key namespacing above are
unchanged.
