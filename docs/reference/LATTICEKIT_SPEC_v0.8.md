...
kind: Kit
relates_to:
  - LATTICEKIT_INTERFACE_v0.8.md  (the API surface this spec contracts)
  - GENIUSLOCUS_ARCHITECTURE_SPEC_v0.8.md  (the lattice as coordinate spine)
  - MDCC_ANNEX_SPEC_v0.1.md  (community classification federation)
  - LATTICE_CITATION_UDC_WIKIDATA.md  (lattice citation decision)
purpose: |
...
- API signatures — those live in `LATTICEKIT_INTERFACE_v0.8.md`.
- Term → code *lookup* (text resolution) — that is EideticLib's job
  (`EIDETICLIB_SPEC_v0.8.md`); LatticeKit defines the space, EideticLib
  resolves into it.
- Community federation / ratification of classification sets — see
  `MDCC_ANNEX_SPEC_v0.1.md`.
