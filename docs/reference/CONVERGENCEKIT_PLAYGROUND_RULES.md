...
**PREREQUISITE (N2 slot registry):** This ordering guarantee holds only
when each device holds a unique HLC node ID. The slot registry (N2,
shipped in the CloudKit backend: slots 1–15 claimed via CloudKit
compare-and-swap, epoch-fenced eviction, loud re-enroll on supersession)
enforces this by assigning a `(slot, epoch)` pair per device at
`enable()` time. Without an active registry, two devices holding the
same node ID make HLC comparisons for the same physical-time window
undefined, and LWW resolves differently on different replicas.
