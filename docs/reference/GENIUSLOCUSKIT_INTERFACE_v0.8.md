...
| Estate kind (provision) | `EstateKind` (`EstateProvision.swift:35`) | `EstateKind` (`rust/src/coordinator.rs:127`) | public / pub | same variants (`glk`/`corpusOnly`/`locusOnly` → `Glk`/`CorpusOnly`/`LocusOnly`); same raw-value strings via `rawValue`/`raw_value` | `provision_lifecycle_parity.rs` / `EstateProvisionLifecycleTests.swift` | Confirmed |
| Provision params | `EstateProvisionParams` (`EstateProvision.swift:73`) | `EstateProvisionParams` (`rust/src/coordinator.rs:172`) | public / pub | identical fields (Swift `Int` → Rust `i64`; snake_case) | `provision_lifecycle_parity.rs` / `EstateProvisionLifecycleTests.swift` | Confirmed |
| Sync mode (provision) | `SyncMode` (`EstateProvision.swift:138`) | `SyncMode` (`rust/src/coordinator.rs:150`) | public / pub | same variants (`none`/`cloudKit`/`federation` → `None`/`CloudKit`/`Federation`); same 0/1/2 storage-mode encoding | `provision_lifecycle_parity.rs` / `EstateProvisionLifecycleTests.swift` | Confirmed |
| Estate mount state | `EstateMountState` (`EstateProvision.swift:166`) | `EstateMountState` (`rust/src/coordinator.rs:183`) | public / pub | same variants (`mounted`/`quiesced`/`draining`/`unmounted` → `Mounted`/`Quiesced`/`Draining`/`Unmounted`) | `provision_lifecycle_parity.rs` / `EstateProvisionLifecycleTests.swift` | Confirmed |
