...
kind: Lib
relates_to:
  - ARIALEXICONLIB_INTERFACE_v0.8.md  (the API surface this spec contracts)
  - GENIUSLOCUS_ARCHITECTURE_SPEC_v0.8.md  (§4.1 storage taxonomy, §5.5 adjective layout, §7.2 acceptance matrix, invariants I-7 / I-8)
  - ARIA_LEXICON.md  (the canonical prose statement of the grammar)
purpose: |
...
GeniusLocusKit, ARIA_MCP) import these types so their surfaces speak one
...
- API signatures — those live in `ARIALEXICONLIB_INTERFACE_v0.8.md`.
- The *values within* each adjective category (e.g. the specific state
  ...
  `GENIUSLOCUS_ARCHITECTURE_SPEC_v0.8.md` §5.5 and reified as value
...
AriaLexiconLib  ← (no dependencies)
      ...
      └── ARIA_MCP          (projects the matrix onto MCP tools)
```

**Depends on:** nothing. Zero-dependency by design — it sits beneath
everything that speaks ARIA.

**Consumed by:** LocusKit, GeniusLocusKit, ARIA_MCP, and any future ARIA
...
(I-1, I-2). The Rust port's `Verb::ALL` and `Adjective::ALL` have the
same lengths.
...
- Adjective *values* (state/trust/sensitivity members) → see
  `GENIUSLOCUS_ARCHITECTURE_SPEC_v0.8.md` § 5.5 and the LocusKit value
  enums.
- Verb *semantics* and dispatch → see `LOCUSKIT_SPEC_v0.8.md` and
  `GENIUSLOCUSKIT_SPEC_v0.8.md`.
- MCP projection of the matrix → see `ARIA_MCP_SPEC_v0.8.md` § 2.
...
*End of AriaLexiconLib Specification v0.8.*
