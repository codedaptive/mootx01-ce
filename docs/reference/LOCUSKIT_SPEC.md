...
anything else refused by version. `mootx01 upgrade` applies that gate first
and skips every later step on a refusal. PersistenceKit's runner enforces
the same rule for every other opener: a stored version inside the ladder's
range with no hop starting at it (a ladder hole — 11–18 here) is refused
with `migrationFailed`, never stamped over.
