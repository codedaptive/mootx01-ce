---
title: IntellectusLib Specification
version: 1.0.0
status: active
date: 2026-06-14
description: "Behavioral specification for IntellectusLib: invariants, conformance requirements, and the contract it guarantees."
spec_type: kit
authors: MOOTx01 maintainers
relates_to:
  - docs/reference/INTELLECTUSLIB_INTERFACE.md
---
...
SubstrateLib) — can emit telemetry without a layering cycle. Those packages
declare a dependency on IntellectusLib; IntellectusLib itself is the
dependency-free leaf at the bottom of the graph.
...
- The `RecentWindowSink` bounded recent-window sink (§ 9, I-8)
...
- Any transport or serialization (a transport sink is a host concern)
- The observer **program** (the env/flag-driven enable loop) — that lives in
  `AriaResident.Observer`; IntellectusLib provides only the window primitive
  (`RecentWindowSink`) it installs. See the aria-mcp specification and the
  ObserverSink documentation.
- The topology worker — the `AutonomicGovernor` topology-snapshot duty consumes
  the observed stream; IntellectusLib only carries the `event` variant it emits.
- Any clock reads — timestamps are always caller-supplied
- Durable batching / SQLite persistence — that is ObserverSink's
  `PersistenceStatsSink`, which `RecentWindowSink` may forward to.
...
IntellectusLib without a cycle.
...
`RecentWindowSink` follows the same discipline: it holds its ring-buffer lock
only for the O(1) buffer mutation and calls its `forward` sink outside that lock.

### I-8: RecentWindowSink is bounded

`RecentWindowSink` retains at most `capacity` samples. On overflow the OLDEST
sample is evicted (FIFO). The window's memory is O(capacity) regardless of how
many samples are received. `capacity` is fixed at construction and clamped to a
minimum of 1. The conformance tests verify the bound holds under overflow and
under concurrent receive: `count` never exceeds `capacity` while `totalReceived`
counts every sample.
...
A topology-worker lifecycle event for the estate-event stream:
...
## § 9 — RecentWindowSink and the observer program

`RecentWindowSink` is a `StatsSink` that keeps a bounded ring buffer of the most
recent samples (I-8) and optionally forwards each sample to a wrapped inner sink.
It is the in-process **recent window** that the resident observer program exposes:
the live proof that emitted samples are not dead letters — a caller reads the
window back and sees the last N samples that crossed the gate.

### § 9.1 Surface

| Member | Semantics |
|---|---|
| `init(capacity:forward:)` / `new(capacity, forward)` | `capacity` clamps to ≥ 1; `forward` (optional) receives each sample after it is recorded |
| `receive(_:)` | Record in the ring (evict oldest if full), then call `forward` outside the lock |
| `snapshot()` | Point-in-time copy, oldest first, ≤ `capacity` samples |
| `count` / `count()` | Samples currently retained (0…capacity) |
| `totalReceived` / `total_received()` | Total received since construction, ignoring eviction (≥ count) |

### § 9.2 Where the observer program and topology worker live

IntellectusLib provides the window **primitive** only. The observer **program**
— the component that decides enablement from config (the `ARIA_MCP_OBSERVER`
env var OR the persisted stats-store monitoring flag), installs a
<!-- ARIA_MCP_OBSERVER retained as a literal runtime env-var name (a code/config identifier consumed by aria-mcp), not a product-name reference. -->
`RecentWindowSink` (forwarding to ObserverSink's `PersistenceStatsSink`), flips
`Intellectus.setEnabled(_:)`, and exposes the window for inspection — lives in
`AriaResident.Observer` (a composition concern, above this leaf library).

The **topology worker** is the `AutonomicGovernor`'s topology-snapshot duty: it
consumes the observed estate stream alongside its periodic recompute and writes
the topology snapshot that `GET /api/graph` serves (turning the
`structurePending` surface true). IntellectusLib only carries the `event`
variant that worker observes — it does not host the worker.

This placement keeps I-1 intact: the window is zero-dependency (Foundation /
std only); everything that touches a store, an env var, or an estate lives above
the leaf.

## § 10 — Conformance requirements (RecentWindowSink)

| Test | Requirement |
|------|-------------|
| `window_records_and_snapshots_oldest_first` | § 9.1 record + snapshot order |
| `bounded_window_evicts_oldest_on_overflow` | I-8 bound + FIFO eviction |
| `capacity_clamps_to_one` | I-8 minimum capacity |
| `forward_sink_receives_every_sample` | § 9.1 forward fidelity (all samples, not just retained) |
| `empty_window_is_empty` | § 9.1 empty state |
| `window_via_gate_enabled_records_disabled_does_not` | I-2 + I-8 via the global gate |
| `concurrent_receive_is_bounded` | I-5 + I-8 bound holds under concurrency |
