...
relates_to:
  - SUBSTRATETYPES_INTERFACE.md  (the API surface this spec contracts)
...
purpose: |
  ...
  anchors, the four matrix carriers, the recall scoring tuple, the
...
- The four matrix carriers (`MatrixF`, `MatrixC`, `MatrixO`,
...
- API signatures — those live in `SUBSTRATETYPES_INTERFACE.md`.
- Hardware-dispatched fast paths for the algebra primitives — those
  live in `SUBSTRATEKERNEL_SPEC.md`.
