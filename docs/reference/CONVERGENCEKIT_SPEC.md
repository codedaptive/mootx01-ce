...
`remoteWakeReceived` when a CloudKit silent-push notification is received
and consumed by the engine (emitted before the nudge() that follows;
CloudKit-only — never emitted by NoSyncEngine or FederationSyncEngine).
Closing the stream stops the subscription. The CloudKit and Federation
backends buffer the newest 256 events.
