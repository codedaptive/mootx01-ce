...
kind: Kit
relates_to:
  - NEURONKIT_INTERFACE_v0.8.md  (the API surface this spec contracts)
  ...
  - COGNITIONKIT_SPEC_v0.1.md  (the not-yet-built consumer that composes NeuronKit into named recipes)
purpose: |
...
not yet built; see `COGNITIONKIT_SPEC_v0.1.md`. NeuronKit itself ships no
user-facing recipes.
...
- API signatures — those live in `NEURONKIT_INTERFACE_v0.8.md`.
...
**Consumed by:** CognitionKit (not yet built). At v0.8 there are **zero
...
fleet convention (one typed error enum per owning module, never optionals
