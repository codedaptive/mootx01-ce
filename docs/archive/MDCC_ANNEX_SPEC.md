...
status: superseded
date: 2026-06-14
description: Annex specification for MDCC community classification federation; superseded by the FDC encoder.
superseded_by: ../reference/FDC_ENCODER_CANONICAL.md
---

> **SUPERSEDED (MDCC→FDC migration).** The MDCC machinery this document describes was removed; the shipped classifier is the FDC encoder — see `docs/reference/FDC_ENCODER_CANONICAL.md` and `docs/engineering/FDC_ENCODER_COOKBOOK.md`. The annex mechanism (ReservedRanges) was removed; an FDC-native annex is a future redesign, not current. Retained for history only.

# MDCC Annex System

## Status

Design spec, draft (the 1.0.0 in front matter is the VERSIONING.md baseline of this draft-era document, not a shipped release). Targets the contribution and conversion layer. Depends on the reserved ranges shipped in MDCC v1 (in LatticeKit, a historical kit name from this draft era), which are in place. Never built.
