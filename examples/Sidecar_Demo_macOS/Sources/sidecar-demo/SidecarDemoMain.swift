...
struct SidecarDemoMain {
    static func main() async {
        ...
        // uses.
        Logging.stderr.log(
            "Sidecar_Demo_macOS starting (in-memory MOOT, stdio MCP)"
        )
        ...
        do {
            ...
            Logging.stderr.log(
                "Sidecar_Demo_macOS fatal: failed to attach MOOT: \(error)"
            )
            exit(1)
        }

        Logging.stderr.log(
            "Sidecar_Demo_macOS ready (\(sidecar.dispatcher.tools.count) tools)"
        )

        let server = StdioServer(dispatcher: sidecar.dispatcher)
        await server.run()

        Logging.stderr.log("Sidecar_Demo_macOS exiting (stdin closed)")
    }
}
